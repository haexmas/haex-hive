...
The scope is additive (target release 4.2.0). Consumers without behavior fragments in their molecule set are unaffected. Every active behavior molecule is also recorded in `.spaex/install.lock`, using the shared composed constitution path as its publication record; `atoms.behavior` is the canonical constitution input and legacy `atoms.constitution` remains read-compatible only.
...
**Storage**: filesystem only. Fragments in `.spaex/constitution.d/<molecule-id>/<fragment-id>.md`. Composed constitution at `<repo-root>/.spaex/constitution.md`. `.spaex/install.lock` records every active behavior molecule with its pinned source, revision, and shared composed-constitution publication path. Clarifications at `.spaex/clarifications.json` (new file, additive; format defined in `contracts/clarifications-schema.md`). Global bootstrap targets are resolved per runtime: Claude's fixed user file, Codex's `$CODEX_HOME` (default `~/.codex`), and Gemini's configured `~/.gemini/<context.fileName>` (paths per runtime; exact rules surveyed in research.md).
