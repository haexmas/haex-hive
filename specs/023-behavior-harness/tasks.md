...
- [X] T072 [US1] Implement post-compose completeness verification in src/spaex/behavior/orchestrate.py: once `invoke_composer` returns a `ComposedShape` (both the direct-compose and the post-clarification-compose path converge before `_commit_behavior_artifacts` runs), parse every `` `<molecule-id>/<fragment-id>` `` citation out of the composed body and diff that set against `canonical_fragments`' scoped ids. A fragment is accounted for if it is cited, or if it is named in `cited_fragments` of any currently-valid persisted clarification (`store.entries.values()` after `invalidate()`). Any fragment satisfying neither raises `ComposerFailureCategory.INVALID_OUTPUT` (exit 32) naming every uncovered fragment, before any artifact is published (FR-012b)
- [X] T073 [P] [US1] Unit test tests/behavior/unit/test_completeness_check.py: a stub composer response that omits one fragment with no clarification on record aborts the build with exit 32 naming that fragment; a stub response that omits a fragment covered by a currently-valid persisted clarification still publishes successfully (FR-012b)
- [X] T074 [P] [US1] Fault-injection test tests/behavior/fault_injection/test_composer_incomplete_output.py: drives the real `orchestrate.run()` transaction (project-local fragments, no git) with a stubbed Composer response that cites every fragment but one, asserting exit 32 and that no `.spaex/constitution.md` is published; a companion case with the dropped fragment covered by a persisted clarification still publishes (FR-012b, SC-011)
...
- [x] ~~T061~~ ~~Update docs/adr/README.md index with entries for 0012 and 0013~~ **Dropped (2026-09-15)**: `docs/adr/README.md` has never existed in this repo, for any of the 21 ADRs across every prior spec. Not a real project convention; not creating one retroactively for spec 023 alone.
- [x] ~~T062~~ ~~Update .specify/memory/constitution.md §"Reserved paths" to add `<repo-root>/.spaex/constitution.md`, `.spaex/constitution.d/`, `.spaex/clarifications.json`, `.spaex/.stale`, `.spaex/composer.log` and align the PATCH-level version bump per Governance §Amendments~~ **Superseded by ADR-0018 (2026-09-12)**: two days after this task was written, ADR-0018 deliberately reset `.specify/memory/constitution.md` (v2.0.0 → v1.0.0) specifically to *remove* spaex-specific reserved-path references and hand that ownership to `.spaex/constitution.md`. Doing this task as written would revert that decision.
- [X] T063 Remove or refactor any existing writer of `.spaex/constitution.md` (spec-007 D2/D16) so no orphan path remains; run grep across src/ for `constitution.md` and audit hits (research.md §11) — audited 2026-09-15: the only other writer is `src/spaex/constitution/publish.py`, the intentional, documented legacy `atoms.constitution` join path (ADR-0010), still actively called from `cli/add.py`/`cli/install.py`. No orphan.
- [X] T064 Update quickstart.md's per-release manual verification block references to point at the current test suite
- [x] ~~T065~~ ~~[P] Bump `spaex.__version__` to 4.2.0 in pyproject.toml and src/spaex/__init__.py~~ **Moot**: 4.2.0 already shipped 2026-09-14 (`85babab`, "chore: release spaex 4.2.0"); main is now at 5.0.0 (`v5.0.0`, tagged 2026-09-15). The version bump for the upcoming patch release covering this work (5.0.1, per Conventional Commits `fix:` → PATCH) happens as part of that release, not as a spec-023 cleanup step.
- [x] ~~T066~~ ~~[P] Add CHANGELOG.md entry for 4.2.0 covering behavior-harness additions, listing every new FR by number~~ **Dropped (2026-09-15)**: `CHANGELOG.md` has never existed in this repo, across any of the 5 tagged releases (v4.0.0 through v5.0.0). Not a real project convention; not creating one retroactively for spec 023 alone.
- [ ] T067 Run the manual verification checklist from quickstart.md §"Manual verification: cross-runtime discoverability" once (SC-007 real-runtime portion); paste observations into the PR description — **needs a human**: real Claude/Codex/Gemini CLI sessions against a scratch account, not executable from an agent session. Left open; not a blocker for the 5.0.1 release.
