...
- Q: The Composer can return a *syntactically valid* Shape A response (correct header, correct `source_hash`/`build_input_hash`, since both are computed from the input and merely echoed back) whose body nonetheless omits one or more input fragments entirely, with no error and no Shape B question. Is that a supported outcome? → A: No. FR-012a's "silent degradation... not supported" already covers the case where the Composer *fails outright* and something falls back to raw concatenation; it does not cover a nominally successful response that is quietly incomplete. That gap is closed by FR-012b: every input fragment must either be cited in the output or be explicitly accounted for by a persisted clarification (i.e. it was surfaced through the Shape B contradiction/overlap path and the operator resolved it — possibly by dropping it). A fragment that is neither cited nor covered by a clarification means the Composer dropped it without going through the resolution path spaex already provides for legitimate omissions, and that MUST abort the build the same way any other invalid-output failure does (FR-012a's mechanism, same exit code), naming every uncovered fragment.
...
2. **Given** the same project after installation, **When** the operator opens `.spaex/install.lock`, **Then** it contains one entry for each active behavior molecule with its pinned source, revision, and the shared `.spaex/constitution.md` publication path.
3. **Given** a fresh project with `.spaex/constitution.md` produced and a runtime that has run the global bootstrap, **When** the operator opens the runtime in the project, **Then** the runtime reads `.spaex/constitution.md` and treats its content as project instructions without any per-project configuration.
4. **Given** an operator switching agent runtime from Claude Code to Codex CLI in the same project, **When** they open the project without further configuration, **Then** the new runtime picks up the same behavior harness by reading the same `.spaex/constitution.md`.
...
- **Behavior-only install lock**: a project whose active molecules contribute only `atoms.behavior` fragments still receives one `.spaex/install.lock` entry per behavior molecule. The lock records the shared composed artifact even though no legacy `atoms.constitution` file is used.
...
- **FR-012b**: After the Composer returns a Shape A response, the system MUST verify completeness before publishing: every input fragment's scoped id (`<molecule-id>/<fragment-id>`) MUST be either (a) cited at least once in the composed body's per-clause provenance annotations (per FR-021), or (b) covered by a currently-valid persisted clarification (post-invalidation per FR-012) whose cited fragments include that scoped id — meaning the fragment was surfaced through the Shape B contradiction/overlap path (FR-005a/FR-010a) and the operator's resolution accounts for its absence. A fragment satisfying neither condition MUST abort the build as an `invalid-output` failure (the same failure category and exit code FR-012a uses), with a diagnostic that names every such fragment by scoped id. This check MUST run before any artifact is published, so a build that fails it leaves `.spaex/constitution.md` and `.spaex/constitution.d/` untouched, exactly as FR-012a requires for any other Composer failure. See Clarification, Session 2026-09-15.
...
- **FR-025**: Every active molecule that contributes one or more `atoms.behavior` fragments MUST have a corresponding entry in `.spaex/install.lock` after a successful `spaex install`, including the install triggered by a clean `spaex add` or `spaex remove`. The entry MUST retain the molecule's pinned source and revision and MUST list `.spaex/constitution.md` as the shared publication path. A contradiction or other deferred internal add/remove plausibility result MUST preserve the last valid lock until a successful full install reconciles it. A behavior-only install MUST NOT leave `molecules` empty.
...
- **Target release**: this spec targets spaex 4.2.0. `atoms.behavior` is the canonical constitution input; the implementation may continue reading legacy `atoms.constitution` declarations for compatibility, but new molecules MUST use behavior fragments. Consumers who adopt behavior fragments opt in by pinning molecules that ship them.
