...
- [X] T060 [P] [US3] Add POSIX unit tests for the manifest-lock helper in `tests/unit/test_manifest_lock.py`: lock file is created if absent and never renamed/deleted; bounded-wait acquisition returns success within the timeout when the lock frees up in time; contended acquisition after the timeout refuses with `manifest-lock-contended` (exit code 6); `--lock-timeout=0` (fail-fast) refuses immediately on contention without waiting; nested acquisition in the same process reuses the held context; kernel-level release on process crash is the sole automatic recovery path (the tool does not force-break locks held by living processes).
