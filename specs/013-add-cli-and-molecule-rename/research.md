...
**Decision**: `haex add` runs `git ls-remote <source-url> HEAD` (or the given ref) to resolve a full SHA, then `git clone --depth 1 --revision <sha>` into a `tempfile.TemporaryDirectory()` under `$HAEX_HIVE_STATE/tmp/`, reads `manifest.json` at repo root, validates it against the v3 publisher-manifest schema, and deletes the temp checkout. If the publisher clone already exists under `$HAEX_HIVE_STATE/repos/<clone-hash>/` for that source, `haex add` fetches into the existing clone rather than creating a temp clone (which primes the on-disk state for the subsequent `haex install`).

**Rationale**: `git clone --depth 1 --revision <sha>` is the simplest correct option; it handles authentication, protocol negotiation, and git-object plumbing without the tool re-implementing them. Ship-and-iterate: measure the performance on real publisher repos before optimizing. Reusing an existing publisher clone when present avoids a redundant network round-trip and warms the cache that `haex install` will consult immediately after.
