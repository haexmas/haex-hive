...
7. **Given** the underlying `haex install` needs review-gated constitution merge (`--llm=file` path), **When** `haex add` runs install, **Then** the manifest edit persists, install produces a review candidate, and the operator can complete adoption with `haex install --accept-merged <candidate>`.
...
- **FR-020**: When the underlying `haex install` requires the review-gated constitution merge path, `haex add` MUST let the manifest edit persist, run install in `--llm=file` mode, and print the review candidate path. Completion is via a follow-up `haex install --accept-merged <candidate>`.
...
- **FR-022**: `haex add` MUST expose the refusal keys `source-url-invalid`, `revision-not-found`, `publisher-manifest-missing`, `molecule-id-not-in-source`, `workflow-molecule-already-adopted`, and `constitution-review-pending`. Each key names the failing input in the diagnostic.
...
- **Two-line adoption for review-gated merges is acceptable at MVP**: `haex add ...` followed by `haex install --accept-merged <candidate>` when a constitution merge requires review. A single-command shortcut may follow if the two-line UX chafes.
...
- **Constitution Principles IV (immutable revisions) and VI (review-gated constitution merges)** ([`.specify/memory/constitution.md`](../../.specify/memory/constitution.md)): `haex add` writes only full SHAs and yields to the `--llm=file` two-phase flow when a constitution merge needs review.
