...
- **FR-004** (Marker as sole publication event): The transaction MUST prepare a staged `.haex-hive.next/visibility.json` marker containing a unique, time-based generation ID and the list of participating output-root names. The rename-swap that renames `.haex-hive.next/` → `.haex-hive/` (per FR-003) is the sole publication event. Idempotent cleanup MAY follow it but MUST never change the marker or the published generation. Per-root and per-file `content_integrity` digests are retired by the 2026-09-01 trust-git amendment (see research §R5); git provides the byte-identity guarantee for anything committed under a participating output root.

- **FR-005** (Reader visibility invariant): A reader MUST first load `visibility.json` to determine the currently-published `generation_id`. A missing marker or a marker that names a generation whose participating-root directories are not fully in place MUST be treated as an unavailable installation, never as a partially-valid one. Byte-identity of the committed content is delegated to git (the trust-git amendment, 2026-09-01); readers that want to detect local tampering use `git status` / `git diff`. Cross-satellite reproducibility comes from the immutable-SHA revision pins in `.haex-hive.json` (Principle IV) plus git's content-addressed delivery, not from digests recorded in `install.lock` or `visibility.json`.
