...
- **FR-002** (In-flight recovery via directory-name state): The transaction MUST NOT keep a separate durable journal file. Instead, the in-flight state of each participating output root MUST be encoded in the presence or absence of three same-filesystem sibling directory names — `<root>/`, `<root>.next/`, and `<root>.prev/` — as defined by research §R1 and §R7. The transaction MUST fsync the parent directory after every rename that transitions between these names. The next `haex install` (or `haex verify --recover`) MUST inspect these three names before building a new plan and complete forward, roll back, or refuse per the state table in §R7. If the fresh `<root>/` is present with a valid `visibility.json` (marker verifies against on-disk digests), recovery MUST NOT roll back that generation; it removes any lingering `<root>.next/`/`<root>.prev/` idempotently and continues.
...
- **In-flight recovery state** — the presence or absence of three same-filesystem sibling directory names beside each participating output root: `<root>/` (live), `<root>.next/` (staged, verified, awaiting swap), `<root>.prev/` (pre-image retained during the swap). This state replaces the earlier durable-journal file; recovery inspects the three names and dispatches per research §R7's state table.
