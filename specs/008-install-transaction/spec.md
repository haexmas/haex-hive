...
- **FR-010** (Stale-lease detection): The transaction MUST implement the following fenced-lease contract so a genuinely-abandoned lock can be safely reclaimed and a live-but-slow install is never wrongly recovered:
  ...
  2. `install.mutex` MUST contain the token, an immutable UTC `acquired_at`, a UTC `heartbeat_at`, `heartbeat_interval_ns = 5_000_000_000`, `ttl_ns = 60_000_000_000`, and `safety_margin_ns = 5_000_000_000`. The owner MUST heartbeat every 5 seconds by atomically replacing and fsyncing the record; monotonic time schedules the owner's heartbeat, while UTC timestamps are used for cross-process expiry.
  3. Recovery MUST first attempt the same non-blocking exclusive OS lock. If a live owner still holds it, recovery MUST wait or refuse, regardless of the recorded age. After acquiring it, recovery MUST read the lease, require `now_utc - heartbeat_at > ttl_ns + safety_margin_ns`, then re-read the record under the exclusive handle and require the token and heartbeat to be unchanged and still expired. Any failed revalidation MUST abort reclamation.
  4. Reclamation MUST atomically replace the lease with the recovering process's new token before replaying or rolling back. Every owner MUST revalidate its token before each mutation; a resumed process with a fenced token MUST stop without touching transaction state. `mtime` alone MUST NOT be treated as sufficient signal.
...
- **FR-021** (Device-local state root): The transaction MUST place `install.mutex` and `install.journal` under `$HAEX_HIVE_STATE/locks/<repo-key>/` — the same device-local state root the constitution-assembly path uses for publisher clones, with the XDG data-directory default defined by the CLI. The in-repo `.haex-hive/` directory MUST remain 100% committed content — no gitignored subpath under it, no lock or journal files inside the repo tree. `<repo-key>` MUST be the lowercase hexadecimal SHA-256 of the canonical, device-independent Spec-007 repo identity (git remote URL for git-backed projects, `.harness-id` for non-git folders). The full canonical identity MUST be stored separately in a state-root `repo-identity.v1.json` record for diagnostics and collision detection; credentials and path separators MUST never appear in the directory name. Multiple checkouts of the same repo on one satellite therefore collide safely on the same lock.

  The path and identity derivation MUST be provided by shared helpers used by `haex install`, `haex constitution assemble`, and `haex constitution show`. Existing `.haex-hive/constitution-transaction.lock` and `.haex-hive/constitution-transaction.json` artifacts are legacy inputs: the first command that obtains the new shared lock MUST recover a valid legacy journal before building a plan, and MUST leave no new legacy artifacts behind. A legacy lock file MAY remain as inert compatibility residue, but no new transaction may create or depend on it.
...
- **Visibility marker (`.haex-hive/visibility.json`)** — the sole publication event. Contains a deterministic generation ID and each participating root's digest. Written as the final publication step inside the transaction; journaled idempotent cleanup may follow without changing the marker or generation.
