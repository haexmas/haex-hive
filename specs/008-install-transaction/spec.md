...
**Independent Test**: With a running `haex install` on a satellite, kill the process (SIGKILL) at each of these journal states: (a) after the exclusive lock is acquired but before any staging; (b) after staging some outputs but before `install.lock` is sealed; (c) after `install.lock` is sealed but before `visibility.json` is published; (d) after `visibility.json` is published but before cleanup. In each case, run `haex install` again and verify recovery either completes the new generation or rolls back to the previous one; verify no reader observed a torn state at any point.
...
- **Mid-install source mutation**: an editor rewrites `.haex-hive.json` while an install is running. The transaction MUST re-read the live inputs before publishing any output, hash them, and abort on any mismatch with the plan snapshot per "Stable staged-input reads through commit". The conformance suite must exercise this.
...
- **The lock file is present but its owner is dead** (crashed, host rebooted, VM paused indefinitely): recovery MUST distinguish "genuinely-abandoned lock" from "live-but-slow install" via the fenced-lease contract chosen in this spec (see clarification below). mtime alone is insufficient.
...
- **FR-002** (Journal + startup recovery): Every filesystem mutation step MUST be recorded in a durable journal BEFORE it is executed. A journal state transition MUST fsync both the journal file and its parent directory before advancing. The next `haex install` (or `haex verify --recover`) MUST replay or roll back any incomplete journal before building a new plan.

- **FR-003** (Repository-wide visibility commit): New bytes MUST be staged next to the target (same filesystem), fsynced, and prepared as complete logical output-root views. Publication MUST occur through the mechanism the source doc's "Repository-wide visibility commit" clause specifies for each root class: for haex-owned roots (`.haex-hive/`), an atomic same-filesystem directory exchange primitive; for mixed-ownership roots (`.claude/`, `.codex/`), an adapter-owned overlay under a versioned generation directory with a stable per-adapter current-generation pointer. The overlay MUST change only paths recorded as adapter-owned; it MUST NEVER enumerate, copy, or replace sibling entries in the mixed-ownership root. A platform lacking an atomic pointer or stable overlay mechanism for a required root MUST refuse the install.

- **FR-004** (Marker as sole publication event): The transaction MUST write and fsync `install.lock` after every other staged output has been sealed, compute participating-root digests over those final staged bytes, then write and fsync a staged `.haex-hive/visibility.json` marker containing a deterministic generation ID and the digest of every participating output root. Readers MUST NOT treat individual root exchanges or pointer replacements as publication. The `.haex-hive/` exchange or pointer replacement containing the final marker MUST be the final journal step and is the sole publication event.

- **FR-005** (Reader visibility invariant): A reader MUST first load `visibility.json` and then verify every participating root's generation and digest. A missing marker or a mixed-generation observation MUST be treated as an unavailable installation, never as a partially-valid one. The `.haex-hive/` digest MUST include `install.lock` and MUST exclude only the marker itself (to avoid self-reference). The digest for a mixed-ownership root MUST cover only its managed overlay, never its unowned directory contents.

- **FR-006** (Stable staged-input reads through commit): Plan-build MUST capture the exact bytes of `.haex-hive.json`, every publisher manifest, and every atom manifest into a sealed plan snapshot with recorded digests. Immediately BEFORE publishing any output, still under the install lock, the transaction MUST re-read live inputs into a fresh commit snapshot, re-hash them, and compare every digest with the plan snapshot. Any mismatch — including a source identity/metadata change during capture — MUST abort the install BEFORE the first output-root swap. Only the digest-matching fresh commit snapshot MUST be used for resolution and hydration; no later step MUST re-read live inputs.
...
- **FR-010** (Stale-lease detection): The transaction MUST implement a fenced-lease contract for lock ownership so a genuinely-abandoned lock can be safely reclaimed and a live-but-slow install is never wrongly recovered. The contract MUST define: a unique owner token stamped at acquisition; a heartbeat/renewal cadence; atomic revalidation of ownership before any recovery action; and recovery only when the lease has demonstrably expired past its safety margin. mtime alone MUST NOT be treated as sufficient signal.
...
- **FR-014**: The conformance suite MUST cover crash recovery from every journal state a `haex install` invocation can enter — at minimum, a crash after each of: (a) lock acquisition; (b) each staged-output seal; (c) `install.lock` fsync but before marker; (d) marker publication but before cleanup.

- **FR-015**: The conformance suite MUST cover a mid-install mutation of `.haex-hive.json` and prove that no output is published — the install aborts at the commit-time re-hash step (FR-006).
...
- **FR-021** (Device-local state root): The transaction MUST place `install.mutex` and `install.journal` under `$HAEX_HIVE_STATE/locks/<repo-identity>/` — the same device-local state root the constitution-assembly path already uses for publisher clones, defaulting to `~/.local/share/haex-hive/` (XDG_DATA_HOME convention). The in-repo `.haex-hive/` directory MUST remain 100% committed content — no gitignored subpath under it, no lock or journal files inside the repo tree. `<repo-identity>` MUST be the same device-independent identity Spec 007 assigns to the project (git remote URL for git-backed projects, `.harness-id` for non-git folders), used verbatim as a filesystem-safe path segment so that multiple checkouts of the same repo on one satellite collide safely on the same lock.
...
- **Visibility marker (`.haex-hive/visibility.json`)** — the sole publication event. Contains a deterministic generation ID and each participating root's digest. Written LAST inside the transaction; the `.haex-hive/` view-swap-with-marker is the final journal step.
