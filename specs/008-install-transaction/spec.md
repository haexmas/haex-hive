...
1. **Given** an interrupted install left `<root>.next/` and/or `<root>.prev/` present, **When** the operator runs `haex install`, **Then** the transaction removes those siblings under the exclusive install lock and reinstalls the fresh generation from the pinned inputs.
...
- **FR-002** (Stale-sibling cleanup before install): The transaction MUST NOT keep a separate durable journal file. For each haex-owned output root, the only durable in-flight state is the presence or absence of two same-filesystem sibling directories, `<root>.next/` and `<root>.prev/`, beside the live `<root>/`. Before building any new plan, the next `haex install` MUST remove any such sibling directories under the exclusive install lock, fsync the parent, and then run the regular deterministic install pipeline. There is no recovery-forward or roll-back path: a crashed install's state is thrown away and the reinstall converges to a valid generation from the pinned inputs. The transaction MUST still fsync the parent directory after every rename that transitions between the three names (§R1). Superseded 2026-09-02 by the detect+retry amendment in research.md §R7; the earlier 8-state recovery dispatcher, `haex verify --recover`, and every "restore from `.prev/`" clause were retired.
...
- **FR-009** (`install.lock` content): `install.lock` MUST record the resolved atom set: `haex_hive_version`, `generated_by`, the `constitution` block (sources + assembled_by), the `atoms[]` array (id + source + revision + contributed_paths per atom), the immutable `generation_inputs[]` identities for participating adapters and tool configurations, and a `visibility_marker.generation_id` cross-reference. `generation_inputs[]` MUST be sorted deterministically and validated against the identities actually used to produce the candidate. Per-file / per-root `content_integrity` fields are retired by the 2026-09-01 trust-git amendment; deterministic generation from pinned inputs provides byte identity for generated payloads. `install.lock` is one of the files inside `.haex-hive.next/` and is published atomically with the whole generation via the rename-swap — there is no separate ordering constraint between it and `visibility.json` (they land together).
