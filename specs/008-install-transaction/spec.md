...
- **FR-002a** (Mixed-root publication ordering): Before the new marker is published, the installer MUST stage and validate every adapter generation, atomically exchange each active adapter pointer, and publish the new `.haex-hive/visibility.json` only after every pointer names the candidate generation. A reader that observes candidate pointers with the prior marker MUST treat the installation as unavailable. If the process stops before marker publication after switching a pointer from prior generation `P` to candidate `C1`, the next `haex install` MUST discard stale staged state and restore `P` before reading inputs or resolving contributions; it MAY then build and publish a fresh candidate `C2`. The retry MUST revalidate the complete candidate, and it MUST NOT leave a pointer naming neither `P` nor the candidate being published. A pointer naming neither the prior nor current candidate MUST be refused without deleting evidence. After marker publication the marker is authoritative; matching candidate pointers are retained and obsolete generations are cleaned only after all pointers match it.
