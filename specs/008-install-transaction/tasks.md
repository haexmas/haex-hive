...
- [ ] T021 [P] [US1] Integration test at [tests/install/integration/test_happy_path.py](../../tests/install/integration/test_happy_path.py): using a fixture repo under `tests/install/fixtures/` (create as part of this task) with two atoms, assert (a) `.haex-hive/constitution.md` matches the byte-for-byte assembled body, (b) `.haex-hive/install.lock` records both atoms' `(id, source, revision, contributed_paths)` — no `content_integrity` fields under the trust-git amendment, (c) `.haex-hive/visibility.json` names a fresh generation ID and lists `.haex-hive/` in `participating_roots`, and (d) `install.lock.visibility_marker.generation_id` equals `visibility.json.generation_id` and both participating-root lists are byte-identical and identically ordered.
...
- [ ] T027 [US1] Implement the time-based generation ID helper (`g_<UTC-ISO8601-basic>_<sha256-prefix4>`) per research §R8 in `src/haex_hive/install/generation.py` (new module — the retired `install/plan.py` no longer exists). Timestamp is UTC allocation time; the 4-hex suffix is `sha256(constitution_body)[:4]` (or another stable per-generation input under the trust-git amendment — no sealed plan digest). The helper is called only after the stable resolved output comparison says publication is needed; allocation MUST advance past an equal existing generation ID so IDs are unique and lexicographically time-ordered.
- [ ] T028 [US1] Implement the slim `install()` entry point in [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py) (replacing the T003 stub). Under the 2026-09-01 trust-git amendment the pipeline collapses to ~50 LoC: acquire exclusive lock via `ConstitutionWriterLock(paths.mutex, OwnerToken.emit())` (T019) → `inflight.resolve(repo_root / HAEX_HIVE_DIR)` (T042) → load `ConsumerManifest.from_json` → `resolve_constitution_contributions` (existing) → compose the stable resolved output projection (T029/T030) → compare it with the live generation after normalising only transaction metadata (`generation_id` and `written_at`). If unchanged, report `"no changes"` and perform no generation-ID allocation, volatile-metadata write, staging, rename, or timestamp update. Only when the stable projection differs, allocate `generation_id` via T027, compose `install.lock` bytes (T029) + `visibility.json` bytes (T030), and call `publish_generation(live_dir, [StagedFile(...), StagedFile(...), StagedFile(...)], state_root, repo_root)` (T020). The independent `[validate_generation_inputs]` pre-seal stage runs after resolution and before T029; it validates the exact identities and profiles that T029 consumes. Depends on T019, T020, T027, T029, T030, T042. No T024 / T025 / T026 dependency — those are retired.
- [ ] T029 [US1] Compose `install.lock` bytes: first run the independent pre-seal `[validate_generation_inputs]` stage on the resolved generation inputs, then build the slimmed `InstallLock` (T016) record from `ConsumerManifest.atoms[]` + the resolved `constitution.sources` + that stage's complete, already-validated generation-input identities and serialization profiles + the exact `participating_roots` list and `generation_id` shared with T030 (T027), sort `generation_inputs` by `(kind, id)` before constructing `InstallLock`, assert that the resulting list is in that order, and serialise via `install_lock.to_json_bytes()`. Expose a stable projection that excludes only the transaction metadata used for no-op comparison. Validate that the lock's root list and `visibility_marker.generation_id` are byte-identical to the values supplied to T030. The serialization profile must be passed unchanged from `[validate_generation_inputs]`; T029 MUST NOT infer tool defaults. Lives inline in `cli/install.py` — no separate module. Depends on T016, T027, T042.
- [ ] T030 [US1] Compose `visibility.json` bytes: `{"haex_hive_version": "2", "generation_id": <T027>, "participating_roots": [".haex-hive/"], "written_at": <ISO 8601 UTC>}`, serialised via `json_deterministic.dumps`. `generation_id` and `written_at` are allocated/written only for a changed publication; the stable projection used by T028 excludes those two transaction metadata fields. Lives inline in `cli/install.py`. Depends on T027.
