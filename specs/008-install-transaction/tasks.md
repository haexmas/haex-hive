...
- [ ] T029 [US1] Compose `install.lock` bytes: build the slimmed `InstallLock` (T016) record from `ConsumerManifest.atoms[]` + the resolved `constitution.sources` + `generation_id` (T027), serialise via `install_lock.to_json_bytes()`, and expose a stable projection that excludes only the transaction metadata used for no-op comparison. Lives inline in `cli/install.py` — no separate module. Depends on T016, T027.
