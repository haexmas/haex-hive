...
- [x] T024 [P] [US1] Implemented `build_plan(repo_root, state_root) -> PlanBuildResult` in [src/haex_hive/install/plan.py](../../src/haex_hive/install/plan.py) as the MVP entry point. Reads `.haex-hive.json` via the existing `ConsumerManifest.from_json`, resolves atoms against publisher clones by reusing `constitution.resolve.resolve_constitution_contributions`, computes SHA-256/base64url-nopad digests of the manifest bytes + each publisher `manifest.json` + each atom `manifest.json`, and seals a `PlanSnapshot` via `PlanSnapshot.seal(...)`. Emits a three-step plan under `.haex-hive/`: `stage_file` (constitution.md with `content_integrity` payload), `seal_install_lock` (install.lock), `publish_marker` (visibility.json). Returns `PlanBuildResult(snapshot, constitution)` so downstream steps (T026 staged writer / T029 seal / T030 marker) can hydrate byte content without re-resolving. **MVP scope**: single-source constitution only. Empty manifest raises `NoSourcesDeclaredError`; multi-source raises `MultiSourceNotSupportedByBuildPlan` (folded together with the LLM-merge path in T031). Overlay pointers (Spec 010) and hook_invoke (Spec 009) not emitted. 6 unit tests in [tests/install/unit/test_build_plan.py](../../tests/install/unit/test_build_plan.py) cover the empty/single/multi shapes plus digest and payload assertions.
