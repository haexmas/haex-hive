...
- [x] T028 [US1] Implemented the slim `install()` entry point in [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py) replacing the T003 stub. Pipeline: acquire the writer lock → clean stale transaction siblings → load the v3 consumer manifest → resolve constitution contributions → perform the single-source no-op check → report `no changes` without writes when the published result already matches, otherwise delegate to `assemble_single_source`, which publishes the constitution and authoritative `install.lock` through `publish_generation`. If resolution finds two or more distinct constitution-contributing molecules, `haex install` refuses with `key=constitution-already-adopted` instead of merging them. The former `assemble_multi_source` path and the retired merge flags are gone. T031 was retired rather than inverted: the `haex constitution assemble` shortcut was removed.
- [x] T029 [US1] `install.lock` composition lives in [src/haex_hive/constitution/assemble.py](../../src/haex_hive/constitution/assemble.py)::`_publish_constitution` (not inlined in `cli/install.py` — see T028). It builds the amended lock from the resolved molecule records, recording each molecule's `id`, `source`, `revision`, and canonical `paths[]`; validates the lock before staging and seals it as the authoritative publication content.
- [x] ~~T030~~ [US1] ~~Compose `.haex-hive/visibility.json` and cross-check its generation with `install.lock`.~~ **Retired by the 2026-09-03 install.lock amendment:** `install.lock` is the sole publication record; there is no separate visibility file or marker/lock cross-check to compose or validate.
...
- [x] T046 [P] [US4] Integration test at [tests/install/integration/test_delete_orphans.py](../../tests/install/integration/test_delete_orphans.py) asserts that a manifest resolving two or more constitution-contributing molecules is refused with exit code 2 and `key=constitution-already-adopted`, and that no publication occurs.
...
- [x] T054 Quickstart walkthrough automated at [tests/install/integration/test_quickstart_walkthrough.py](../../tests/install/integration/test_quickstart_walkthrough.py). Two tests execute the quickstart steps in order against real fixtures (single-source for steps 1/2/4/5/7, multi-source for step 6) plus a skip-marker for step 3 (`--verify-only`, deferred to T037). **Retired 2026-09-04 (ADR 0010)**: step 6's multi-source case asserts refusal (`key=constitution-already-adopted`) rather than a merged install; there is no multi-source success path. **Doc-fixes landed alongside** in [quickstart.md](./quickstart.md): dropped fabricated count and stale generation-status text, scoped the no-op guarantee to the published `.haex-hive/` tree, marked step 3 deferred, corrected the step-4 refusal to `key=constitution-writer-busy` (exit 9), removed the obsolete ownership claim, and replaced deferred lease prose with a T034 note. **SC outcomes**: SC-001 crash-safety and retry convergence; SC-002 exclusive-lock serialisation; SC-003 byte-identical no-op; SC-004 coordinated-input-writer invariant (covered by `test_transaction.py::test_concurrent_writer_refused`); SC-005 platform-primitive availability deferred to Spec 010; SC-006 unowned-file survival deferred until mixed-root adapters; SC-007 retired by the 2026-09-02 trust-git amendment.
- [x] T055 Full sweep `uv run pytest -q -m ''` (repository test suite): 247 passed, 1 skipped. Ruff is clean for `src` and `tests`.
