...
- [x] T032 [P] [US2] Fenced-lease unit tests in [tests/install/unit/test_fenced_lease.py](../../tests/install/unit/test_fenced_lease.py) — 15 tests covering: `HeartbeatThread` cadence + failure isolation + start/stop idempotency; `attempt_reclaim` handles missing/empty mutex, refuses within TTL, permits after TTL + safety margin, aborts on owner-token change between reads, aborts on heartbeat change between reads; `SharedReaderLock` permits multiple readers and is excluded by an exclusive writer (POSIX-only); `busy_lock_from_mutex` extracts operator diagnostic + degrades gracefully on missing/corrupt mutex.
...
- [x] T034 [US2] Implemented in [src/haex_hive/install/lock.py](../../src/haex_hive/install/lock.py). `HeartbeatThread(lock, interval_ns=5s)` — daemon thread that calls the lock's `heartbeat()` primitive at the configured cadence; on heartbeat failure it stops rather than spinning so the lease expires naturally. `attempt_reclaim(mutex_path, *, ttl_ns=60s, safety_margin_ns=5s, now_ns=None)` — reads `install.mutex`, requires the wall-clock heartbeat to be older than `ttl_ns + safety_margin_ns`, re-reads under the same handle, requires owner_token + heartbeat_ns to be unchanged between reads. Raises `StaleLeaseNotReclaimable` on any revalidation failure so callers can distinguish "safe to reclaim" from "keep waiting". `time.monotonic_ns()` is used only for the heartbeat scheduler and the R4 `start_ns` diagnostic field; expiry uses `time.time_ns()` per FR-010(2). [cli/constitution.py::run_assemble](../../src/haex_hive/cli/constitution.py) now starts a `HeartbeatThread` on every acquisition and stops it in the finally block.
- [x] T035 [US2] `SharedReaderLock` in [src/haex_hive/install/lock.py](../../src/haex_hive/install/lock.py). POSIX: `fcntl.flock(LOCK_SH | LOCK_NB)` on a `O_RDONLY | O_CREAT` handle. Windows: `LockFileEx` **without** `LOCKFILE_EXCLUSIVE_LOCK`, scoped to the same sentinel byte at offset `0x7FFF_FFFF` that the exclusive lock uses so multi-reader/single-writer semantics land consistently across platforms. On contention with an active writer, raises `BlockingIOError` for the caller to translate. Two POSIX unit tests confirm the multi-reader coexistence and writer-excludes-readers cases; Windows concurrency proof deferred to a follow-up (Windows CI not yet available in this repo).
- [x] T036 [US2] `busy_lock_from_mutex(mutex_path, *, now_ns=None)` in [src/haex_hive/install/errors.py](../../src/haex_hive/install/errors.py). Parses `install.mutex` (best-effort — missing / empty / corrupt JSON degrades to `"mutex metadata unavailable"`), extracts pid + hostname + acquired_at from the `owner_token` colon fields, computes heartbeat age from `heartbeat_at_ns_wallclock`, and formats `"lock owned by pid <p>@<h> since <acquired_at> (heartbeat Xs ago, ttl Ys)"` per the CLI-contract exit-code-9 diagnostic. Returns an `InstallLockBusy` (exit code 9) with `context={"pid", "hostname", "acquired_at", "mutex_path"}` so the diagnostic emitter can render deterministically. [cli/constitution.py::run_assemble](../../src/haex_hive/cli/constitution.py) catches `ConstitutionWriterBusyError` from `ConstitutionWriterLock.__enter__` and re-raises as this formatted `InstallLockBusy`. `tests/integration/test_assemble_single_source.py::test_active_writer_is_excluded` updated to expect `key=install-lock-busy` instead of the old `key=constitution-writer-busy`.
