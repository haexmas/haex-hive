...
- [ ] T019 Extend [src/haex_hive/io/writer_lock.py](../../src/haex_hive/io/writer_lock.py) with an exclusive-with-owner-metadata lock primitive (`fcntl.flock(LOCK_EX | LOCK_NB)` on POSIX, `msvcrt.locking(fd, LK_NBLCK, 1)` on Windows) that reads/writes the `install.mutex` layout defined in data-model.md. Existing constitution-writer callsites keep working; new callers pass an `OwnerToken` (from T013) at acquisition. Depends on T013 and T017.
- [ ] T020 Generalise [src/haex_hive/io/transaction.py](../../src/haex_hive/io/transaction.py) to accept a multi-participant plan (list of per-file staged writes) instead of a single-file assemble; preserve the constitution-assemble callsite as a single-participant special case per research §R9. Depends on T014 and T018.
...
- [ ] T024 [P] [US1] Implement plan-build in [src/haex_hive/install/plan.py](../../src/haex_hive/install/plan.py): read `.haex-hive.json` (reuse Spec 007's `atoms[]` loader), resolve atoms against publisher clones under `$HAEX_HIVE_STATE/repos/`, seal a PlanSnapshot with all input digests, and emit an ordered PlanStep list (`stage_file` per contributed file, one `seal_install_lock`, one `publish_marker`). Depends on T012, T018.
- [ ] T025 [P] [US1] Implement commit-snapshot re-read + digest match in [src/haex_hive/install/commit_snapshot.py](../../src/haex_hive/install/commit_snapshot.py) per FR-006: re-open the same input files, hash again, compare with PlanSnapshot; on mismatch raise `CommitSnapshotMismatch` (T017). Depends on T012.
...
- [ ] T031 [US1] Route existing `haex constitution assemble` through the install pipeline as a constitution-only plan filter in [src/haex_hive/cli/constitution.py](../../src/haex_hive/cli/constitution.py) per research §R9: the CLI shortcut still exists but internally invokes the T028 pipeline with a scope filter; verify existing constitution unit + integration tests pass unchanged. Depends on T028.
...
- [ ] T032 [P] [US2] Unit test at [tests/install/unit/test_fenced_lease.py](../../tests/install/unit/test_fenced_lease.py): heartbeat thread refreshes `heartbeat_at_ns` every 5s (fake clock); TTL of 60s honoured; revalidation-before-reclaim aborts when the owner token changes between the two reads per research §R4.
...
- [ ] T034 [US2] Implement fenced-lease heartbeat thread + revalidation-before-reclaim in [src/haex_hive/install/lock.py](../../src/haex_hive/install/lock.py) per research §R4: 5s cadence, 60s TTL, revalidation ordering (read stale → exclusive-re-read stale-and-unchanged → reclaim). Uses `time.monotonic_ns()`; wall-clock skew allowance 5s. Depends on T013, T019.
...
- [ ] T049 [US4] Implement delta computation in [src/haex_hive/install/delta.py](../../src/haex_hive/install/delta.py): compare previous install.lock's `atoms[].contributed_paths` with new plan's contributed paths; emit `delete_orphan` PlanSteps for each removed path. Depends on T012, T016.
