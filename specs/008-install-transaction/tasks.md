...
- [x] ~~T031~~ [US1] ~~Route existing `haex constitution assemble` through the new `install()` (T028): the CLI shortcut still exists but internally calls the same `install()` entry point.~~ **Retired**: the `haex constitution assemble` CLI subcommand was removed entirely rather than shimmed. Under the pre-user policy the shim carried no adoption benefit, and keeping two entry points doing the same work doubled the test surface. `haex install` at the time owned the `--llm` and `--accept-merged` flags directly; `constitution/assemble.py`'s `_publish_constitution` / `assemble_single_source` / `assemble_multi_source` functions were the composer layer that `cli/install.py::run` called. **Retired 2026-09-04 (ADR 0010)**: the flags and `assemble_multi_source` are gone; only `_publish_constitution` / `assemble_single_source` remain.
...
**Goal**: Two concurrent `haex install` invocations against the same checkout never both succeed; the loser exits with owner detail (PID, hostname, start time) per FR-001. `haex verify` shares reads; installs take the exclusive lock.
...
- [ ] T035 [US2] Add shared-read (`fcntl.flock(LOCK_SH | LOCK_NB)` on POSIX / Win32 `LockFileEx` without `LOCKFILE_EXCLUSIVE_LOCK` on Windows) support in [src/haex_hive/install/lock.py](../../src/haex_hive/install/lock.py) so `--verify-only` acquires shared while `install` acquires exclusive per FR-001. Add a Windows concurrency test proving two readers coexist while a writer is excluded until both release. Depends on T019.
...
**Goal**: A crash at any legal in-flight state is safe to retry: stale staging is removed, the previous generation is retained until a replacement is successfully published, and unowned files under `.claude/`/`.codex/` survive byte-identically.

**Independent Test**: Per [spec.md §US3 Independent Test](./spec.md) — scripted-kill at each rename-swap boundary in FR-014, then run `haex install` and assert deterministic convergence. Also make resolution fail after `rename_a` and assert that `.prev/` remains available for a later retry.

### Tests for User Story 3

- [x] ~~T038~~ [P] [US3] ~~Unit test at `tests/install/unit/test_inflight_recovery.py`: construct each of the eight `<root>{,.next,.prev}` combinations from the §R7 state table and assert the dispatcher reaches the specified outcome.~~ **Retired by the 2026-09-02 detect+retry amendment** (research.md §R7): the 8-row dispatcher is gone, replaced by a single `clean_stale_siblings` helper whose 5 unit tests live in `tests/unit/test_transaction.py` (no siblings; only `.next`; only `.prev`; both siblings from a mid-swap crash; steady live). No conformance-level state-matrix test is needed under the reinstall-converges model.
- [x] T039 [P] [US3] Crash-safety sweep landed at [tests/integration/test_crash_safety.py](../../tests/integration/test_crash_safety.py). Parametrised over the rename-swap boundaries (`pre_swap`, `rename_a`, `rename_b` with and without a preexisting generation); each case runs `haex install` in a child process and terminates it with `SIGKILL` (POSIX) or the platform equivalent (Windows) via the `HAEX_HIVE_CRASH_AFTER` seam. Under the 2026-09-02 detect+retry amendment, the retry step is just another `haex install`; it removes stale `.next`, retains `.prev` until successful publication, and the third run is idempotent. The `rename_a`/preexisting case also proves that a resolution failure preserves `.prev` for a later retry.
...
- [x] T042 [US3] Simplified under the 2026-09-02 detect+retry amendment. `install/inflight.py` now exposes a single `clean_stale_siblings(live)` helper that removes a leftover `<root>.next/` while retaining `<root>.prev/` under the exclusive install lock, then the regular install pipeline runs. The 8-row `InflightState.inspect`/`resolve` dispatcher, `InflightIntegrityError`, and per-state recovery actions are retired; readers get atomic visibility from the rename-swap alone, and the deterministic install pipeline converges on retry. `cli/install.py::run` calls `clean_stale_siblings` before loading the manifest.
- [x] ~~T043~~ [US3] ~~Rollback via `<root>.prev/` restoration.~~ **Retired by the 2026-09-02 detect+retry amendment**. There is no separate recovery command or recovery-forward dispatcher. A crashed install retains `.prev/` until the replacement passes resolution, staging, and post-write validation; successful publication then removes it. A failed retry preserves the previous generation for a later retry.
- [x] ~~T044~~ [US3] ~~Add `--recover` handling to `haex verify`.~~ **Retired by the 2026-09-02 detect+retry amendment**. There is no separate recovery command: a plain `haex install` acquires the exclusive lock, cleans stale siblings via `clean_stale_siblings`, and reinstalls to a valid generation. Operators asking "how do I recover?" get the same answer as npm: `rm -rf .haex-hive.next .haex-hive.prev && haex install` at worst, but the automatic cleanup makes even that unnecessary in practice.
- [x] T045 [US3] Stale-directory cleanup landed as `install.inflight.clean_stale_siblings(live)` — the sole retry-preparation primitive under the 2026-09-02 detect+retry amendment. Called by `cli/install.py::run` after acquiring the exclusive lock and before loading the consumer manifest. Removes `<root>.next/`, retains `<root>.prev/` until successful publication, fsyncs the parent when cleanup occurs, and returns `(next_removed, prev_present)` for diagnostics. Coverage: unit tests in `tests/unit/test_transaction.py` plus the crash-boundary sweep in `tests/integration/test_crash_safety.py`.

**Checkpoint**: Every crash point in FR-014 converges on retry without a torn state; a failed retry preserves the only usable previous generation. Unowned files survive.
...
- [x] T046 [P] [US4] Integration test at [tests/install/integration/test_delete_orphans.py](../../tests/install/integration/test_delete_orphans.py). Two tests cover the drop path against the `multi_source_constitution_fixture`: (a) start with both atoms via `--llm=file` + `--accept-merged`, drop `atom_id_b`, reinstall — assert the new `constitution.md` is byte-for-byte atom-a's contribution (no merged content), `install.lock.constitution.sources[]` shrinks from two entries to one, and `visibility.json.generation_id` advances; (b) after that drop+reinstall converges, a third install reports `no changes` and leaves `install.lock` byte-identical. **Retired 2026-09-04 (ADR 0010)**: this test file was rewritten to assert that a multi-source manifest refuses outright (`key=constitution-already-adopted`) rather than merging; the `--llm=file`/`--accept-merged` two-step described here no longer exists. **Deviation from original wording**: under the constitution-only MVP the removed atom does not contribute its own physical file (both atoms feed the merged `constitution.md`), so the "removed atom's files are gone from every participating root" assertion collapses into the byte-content check plus the atomic R1 rename-swap guarantee that any file only the removed atom would have written is absent from the new `.haex-hive/`. When Spec 010 adapters add per-atom files under mixed-ownership roots, a follow-up test lands there.
...
- [x] ~~T048~~ [P] [US4] ~~Conformance test at `tests/install/conformance/test_partial_delete_rollback.py`: crash mid-swap on a delete install, recover forward via T042.~~ **Retired by the 2026-09-02 detect+retry amendment**. There is no per-delete rollback path to test: under R1 the whole generation is materialised in `.haex-hive.next/` before rename A, and a crash between the two renames is handled by `clean_stale_siblings` + reinstall, which produces the removed atom's absence anyway (the manifest is the source of truth). Coverage is subsumed by [tests/integration/test_crash_safety.py](../../tests/integration/test_crash_safety.py) — the crash-boundary sweep already exercises `rename_a`/`rename_b` under a manifest change.
...
- [x] T054 Quickstart walkthrough automated at [tests/install/integration/test_quickstart_walkthrough.py](../../tests/install/integration/test_quickstart_walkthrough.py). Two tests execute the quickstart steps in order against real fixtures (single-source for steps 1/2/4/5/7, multi-source for step 6) plus a skip-marker for step 3 (`--verify-only`, deferred to T037). **Retired 2026-09-04 (ADR 0010)**: step 6's multi-source case was rewritten to assert refusal (`key=constitution-already-adopted`) rather than a merged install; there is no more multi-source *success* path to walk. **Doc-fixes landed alongside** in [quickstart.md](./quickstart.md): dropped the fabricated `(N atoms, N files)` counts on steps 1 and 6, dropped the `; generation g_… is up to date` tail on step 2, scoped the SC-003 no-op guarantee to the published `.haex-hive/` tree, marked step 3 as deferred, corrected the step-4 refusal to `key=constitution-writer-busy` (exit 9) and removed the obsolete ownership claim, replaced the fenced-lease heartbeat prose with a T034 deferral note, and documented stable marker/lock reads across a rename swap. **SC outcomes**: SC-001 crash-safety (verified by step 5's rename_a-boundary crash → recovered constitution content and matching marker/lock metadata, then clean_siblings + reinstall converge); SC-002 concurrent-install serialisation (verified by step 4's exclusive-lock refusal); SC-003 idempotence (verified by step 2's byte-identical no-op); SC-004 coordinated-input-writer (not exercised — the writer-lock discipline is a spec invariant, not walkable at the CLI level; covered by `test_transaction.py::test_concurrent_writer_refused`); SC-005 platform-primitive availability (not exercised on Linux CI — Windows-Developer-Mode refusal is a Spec 010 concern); SC-006 unowned-files-survive (not exercised — `.claude/`/`.codex/` roots not yet materialised in MVP, Spec 010); SC-007 retired by the 2026-09-02 trust-git amendment.
- [x] T055 Full pytest sweep (`pytest tests/`: 292 passed, 1 skipped, 9 deselected) confirms no regressions after all the amendments. Ruff clean across `src` and `tests`. **Dead symbols pruned**: (a) all six classes in [src/haex_hive/install/errors.py](../../src/haex_hive/install/errors.py) (`HaexInstallError`, `InstallLockBusy`, `IncompleteTransaction`, `CommitSnapshotMismatch`, `OverlayUnsupported`, `SealMismatch`) — zero runtime raisers after the trust-git and detect+retry amendments; the diagnostic keys they carried are covered by sibling classes in `haex_hive.util.errors`. Module retained as a reserved slot for future US2 fenced-lease and Spec 010 adapter errors. (b) `ConstitutionIntegrityMismatchError` in [src/haex_hive/util/errors.py](../../src/haex_hive/util/errors.py) — was tied to the retired `haex constitution show` integrity check (trust-git amendment). No transient imports remained; ruff's `F401` scan across the install subpackage passed cleanly.
- [ ] ~~T056~~ [deferred] ~~Make the adapter `revision` in `generation_input_identities()` at `src/haex_hive/constitution/llm.py:67-91` stable across install methods...~~ **Retired 2026-09-04 (ADR 0010)**: `constitution/llm.py` and `generation_input_identities()` were deleted along with the rest of the multi-source LLM merge. There is no adapter-revision identity left to stabilize.
...
# Then T028 (pipeline orchestration), then T029/T030 finalise. T031 retired: `haex constitution assemble` was removed entirely rather than shimmed.
```
