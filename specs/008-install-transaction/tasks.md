...
- [ ] T029 [US1] Compose `install.lock` bytes: build the slimmed `InstallLock` (T016) record from `ConsumerManifest.atoms[]` + the resolved `constitution.sources` + the generation-input identities + the exact `participating_roots` list and `generation_id` shared with T030 (T027), serialise via `install_lock.to_json_bytes()`, and expose a stable projection that excludes only the transaction metadata used for no-op comparison. Validate that the lock's root list and `visibility_marker.generation_id` are byte-identical to the values supplied to T030. Lives inline in `cli/install.py` — no separate module. Depends on T016, T027.
