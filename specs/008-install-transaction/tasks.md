...
- [x] T042 [US3] Simplified under the 2026-09-02 detect+retry amendment. `install/inflight.py` now exposes a single `clean_stale_siblings(live)` helper (~20 LoC) that removes any leftover `<root>.next/` or `<root>.prev/` sibling under the exclusive install lock, then the regular install pipeline runs. The 8-row `InflightState.inspect`/`resolve` dispatcher, `InflightIntegrityError`, and per-state validation-before-cleanup requirements are all retired; readers get atomic visibility from the rename-swap alone, and the deterministic install pipeline converges on retry. `cli/install.py::run` calls `clean_stale_siblings` before loading the manifest.
- [x] ~~T043~~ [US3] ~~Rollback via `<root>.prev/` restoration.~~ **Retired by the 2026-09-02 detect+retry amendment**. There is no rollback step — a crashed install's `.prev/` is simply deleted along with any stray `.next/` by `clean_stale_siblings`, and the next `haex install` reinstalls from the deterministic pinned inputs. If a crash left `.haex-hive/` missing entirely (crash between rename A and rename B), the retry runs the first-install path and rebuilds the generation from scratch. No data loss under the deterministic-generation contract.
...
- [x] T045 [US3] Stale-directory cleanup landed as `install.inflight.clean_stale_siblings(live)` — the sole recovery primitive under the 2026-09-02 detect+retry amendment. Called by `cli/install.py::run` after acquiring the exclusive lock and before loading the consumer manifest. Removes any `<root>.next/` or `<root>.prev/` sibling, fsyncs the parent, returns `(next_removed, prev_removed)` for diagnostics. Coverage: 5 unit tests in `tests/unit/test_transaction.py` plus the crash-boundary sweep in `tests/integration/test_crash_safety.py`.
