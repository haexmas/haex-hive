...
- [ ] T016 [P] Implement `InstallLock` v2 model + serialiser in [src/haex_hive/model/install_lock.py](../../src/haex_hive/model/install_lock.py) — promote `atoms`, `participating_roots`, `visibility_marker`, and `ownership` to first-class fields per research §R9, replacing the `unknown_top_level` working-buffer pattern currently used for these Spec 008 fields. Under the project's pre-user policy (see `haex_hive_pre_user.md`), reading a Spec 007-vintage record MUST fail schema validation with `InstallLockSchemaInvalidError`; there is no in-tool migration path. Operator recovery is to remove `.haex-hive/install.lock` and re-run `haex constitution assemble`.
...
- [ ] T019 Extend [src/haex_hive/io/writer_lock.py](../../src/haex_hive/io/writer_lock.py) with shared device-local path helpers and exclusive-with-owner-metadata locking (`fcntl.flock(LOCK_EX | LOCK_NB)` on POSIX, Win32 `LockFileEx` with `LOCKFILE_EXCLUSIVE_LOCK` on Windows) that reads/writes the `install.mutex` layout defined in data-model.md, including in-place wall-clock heartbeat updates. Derive the canonical project identity from the git remote or `.harness-id`, hash it to the filesystem-safe `<repo-key>`, derive a device-local `<checkout-key>` from the resolved checkout path, and store the full identity in `repo-identity.v1.json`. Existing constitution-writer callsites use the new paths while retaining the legacy lock as an exclusion guard for older writers; legacy journals are not discovered or migrated by the new transaction. New callers pass an `OwnerToken` (from T013) at acquisition. Depends on T013 and T017.
- [ ] T020 Generalise [src/haex_hive/io/transaction.py](../../src/haex_hive/io/transaction.py) to accept a multi-participant plan (list of per-file staged writes) instead of a single-file assemble; use the shared device-local journal path and recover only the checkout-scoped journal before opening a new transaction. Legacy constitution journals remain untouched for operator cleanup, and the constitution-assemble callsite remains a single-participant special case per research §R9. Depends on T014 and T018.
...
- [ ] T028 [US1] Implement pipeline orchestration in [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py) (replacing the T003 stub): acquire exclusive lock (T019) → recover the checkout-scoped journal before planning (T042 when available; legacy artifacts remain for operator cleanup) → build plan (T024) → verify and seal commit inputs (T025) → resolve/hydrate from the sealed snapshot → stage all outputs (T026), including ownership-based deletes → seal install.lock via T020 → publish visibility.json marker (T030) → journal and perform idempotent cleanup. Report "no changes" when the pre-install marker's `install_lock_content_integrity` equals a dry-run recomputation. Depends on T019, T020, T024, T025, T026, T027, T029, T030, and T042 for recovery-aware constitution routing.
...
- [ ] T031 [US1] Route existing `haex constitution assemble` through the install pipeline as a constitution-only plan filter in [src/haex_hive/cli/constitution.py](../../src/haex_hive/cli/constitution.py) per research §R9: the CLI shortcut still exists but internally invokes the T028 pipeline with a scope filter, holds the legacy lock only as an exclusion guard, uses the shared device-local lock/journal helpers, and recovers only the checkout-scoped journal; legacy constitution artifacts remain for operator cleanup. Verify existing constitution unit + integration tests pass unchanged. Depends on T028.
