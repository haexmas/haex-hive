...
- [ ] T028 [US1] Implement pipeline orchestration in [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py) (replacing the T003 stub): acquire exclusive lock (T019) → dispatch the in-flight recovery state (T042) before planning → build plan (T024) → verify and seal commit inputs (T025) → resolve/hydrate from the sealed snapshot → stage all outputs into `.haex-hive.next/` (T026) → write install.lock (T029) and visibility.json (T030) into `.haex-hive.next/` → invoke the rename-swap primitive (T020) → cleanup `.haex-hive.prev/`. Report "no changes" when the pre-install marker's `install_lock_content_integrity` equals a dry-run recomputation of the same content (no swap needed). Depends on T019, T020, T024, T025, T026, T027, T029, T030, T042.
...
**Goal**: A crash at any legal in-flight state either completes the interrupted install to a valid new generation (row 3 of §R7) or restores the previous generation (rows 2 and 4). Unowned files under `.claude/`/`.codex/` survive recovery byte-identically.
...
- [ ] T042 [US3] Implement the in-flight recovery dispatcher in `src/haex_hive/install/inflight.py` per research §R7: read `os.listdir(parent_of_root)`, filter for `<root>{,.next,.prev}`, map the combination to the §R7 state table, and dispatch (complete forward, cleanup, refuse). Fsync the parent directory after every rename or removal. Replaces the earlier journal-replay implementation. Depends on T018 (digest re-verify on the completed forward path).
- [ ] T043 [US3] Rollback is a direct consequence of the R1 rename-swap contract: `<root>.prev/` retained beside the live `<root>/` IS the pre-image. Restore = `os.rename(<root>.prev, <root>)`. This is folded into T042's dispatcher (there is no separate rollback module). This task-slot is retained to record the design decision; no new file lands. Depends on T042.
- [ ] T044 [US3] Add `--recover` handling to `haex verify` in [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py): acquires exclusive lock (T019), dispatches the in-flight recovery state via T042, reports "recovered generation `<gen>`" (either completed forward or restored from `<root>.prev/`), exit 0. Depends on T028, T042.
...
- [ ] T048 [P] [US4] Conformance test at [tests/install/conformance/test_partial_delete_rollback.py](../../tests/install/conformance/test_partial_delete_rollback.py) (FR-016): inject `SystemExit(137)` mid-swap of a delete-orphans install (after rename A on `.haex-hive/`, before rename B); recover via T042; assert either the swap completes forward (removed atom's files are gone in the new `.haex-hive/`) or the rollback restores the previous `.haex-hive/` with the removed atom's files intact. Under R1 there is no per-delete atomicity concern — the whole generation is materialised in `.haex-hive.next/` and committed by the swap.
