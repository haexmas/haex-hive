...
- [ ] T042 [US3] Implement the in-flight recovery dispatcher in `src/haex_hive/install/inflight.py` per research §R7: read `os.listdir(parent_of_root)`, filter for `<root>{,.next,.prev}` on haex-owned roots, verify candidate markers/digests, and dispatch complete-forward, cleanup, restore from a verified `<root>.prev/` only in the explicit rollback rows, or refuse. A present but invalid `.next` is an integrity failure and must refuse without publication. Fsync the parent directory after every rename or removal. Mixed-root pointer recovery follows R3. Replaces the earlier journal-replay implementation. Depends on T018 (digest re-verification on forward and rollback paths).
