...
- [ ] T039 [P] [US3] Conformance test at [tests/install/conformance/test_crash_matrix.py](../../tests/install/conformance/test_crash_matrix.py) (FR-014): parametrise over the crash points (after lock, after staging, after install.lock, after marker, after `cleanup_started`, and after `cleanup_completed`); each case runs an install, injects `SystemExit(137)` at the target state, then runs `haex install --recover` and asserts the outcome matches one of the two allowed per FR-011 (SC-001), preserving the published generation for the two post-marker cleanup points.
