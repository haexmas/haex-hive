...
**Storage**: Filesystem-only. Reads and writes: `.haex-hive.json` (input, Spec 007 schema), publisher-cloned git objects under `$HAEX_HIVE_STATE/repos/<clone-hash>/` (input, Spec 007's convention), staged bytes under `<root>.staging.<gen>/` siblings of each participating output root, and the participating output roots themselves (`.haex-hive/`, `.claude/`, `.codex/`, plus any Spec 010 adapter roots). Device-local: `install.mutex`, `install.journal`, and `repo-identity.v1.json` under `$HAEX_HIVE_STATE/locks/<repo-key>/` per FR-021.
...
4. **Fenced-lease numeric contract** (FR-010):
   ...
   - **Lease record**: `install.mutex` stores the token, immutable UTC `acquired_at`, UTC `heartbeat_at`, `heartbeat_interval_ns=5_000_000_000`, `ttl_ns=60_000_000_000`, and `safety_margin_ns=5_000_000_000`. The owner atomically replaces and fsyncs it every 5 seconds.
   - **Revalidation ordering**: recovery first acquires the same non-blocking exclusive OS lock; if a live owner holds it, recovery waits or refuses regardless of age. Once acquired, it requires UTC heartbeat age greater than TTL plus safety margin, re-reads the record under the lock, verifies unchanged token and heartbeat, and atomically fences the old token before recovery. `mtime` is never sufficient evidence.
...
7. **Journal replay semantics**: journal is append-only, entries are one JSON-per-line, each entry ends with a SHA-256 tail hash of `<line-content>\n<prev-tail-hash>` for tamper-detection. On recovery, replay forwards from the last committed marker or roll back to the last known-good marker generation; after a verified marker, recovery performs cleanup only.
