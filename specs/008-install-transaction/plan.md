...
Technical approach (from research): extend the existing Python 3.10+ stdlib-first `haex` CLI (Spec 007) with a new `haex install` subcommand plus supporting modules under `src/haex_hive/install/`. Reuse the durable-journal + atomic-write primitives already present in `src/haex_hive/io/` for constitution assembly rather than parallel-implementing them — the constitution assemble path becomes the first participant of a generalised install transaction. Device-local transaction artefacts use a repository-wide mutex at `$HAEX_HIVE_STATE/locks/<repo-key>/install.mutex` and a checkout-scoped journal at `$HAEX_HIVE_STATE/locks/<repo-key>/checkouts/<checkout-key>/install.journal` per FR-021; the full canonical identity is stored separately for diagnostics and collision detection. Same-filesystem staging next to each output root; per-file `os.replace()` for atomic file publication; visibility marker publication is the final publication boundary, after which only journaled idempotent cleanup may run. Mixed-ownership overlay uses OS-native pointer primitives (POSIX symlink, Windows directory junction); refusal on platforms lacking a supported primitive per FR-003.

## Technical Context

**Language/Version**: Python 3.10+ (matches Spec 007's baseline).
**Primary Dependencies**: `jsonschema>=4.18` (already in Spec 007), Python stdlib for everything else — `json`, `hashlib`, `pathlib`, `os` (`replace`, `fsync`, `open`), `fcntl` (POSIX advisory locks), `ctypes` (Windows `LockFileEx`/`UnlockFileEx`), `subprocess` (for `git show`, already used by Spec 007), `dataclasses`, `contextlib`, `secrets` (for owner-token generation). No new external dependency.
**Storage**: Filesystem-only. Reads and writes: `.haex-hive.json` (input, Spec 007 schema), publisher-cloned git objects under `$HAEX_HIVE_STATE/repos/<clone-hash>/` (input, Spec 007's convention), staged bytes under `<root>.staging.<gen>/` siblings of each participating output root, and the participating output roots themselves (`.haex-hive/`, `.claude/`, `.codex/`, plus any Spec 010 adapter roots). Device-local: the repository mutex and identity record under `$HAEX_HIVE_STATE/locks/<repo-key>/`; each checkout's journal is under `$HAEX_HIVE_STATE/locks/<repo-key>/checkouts/<checkout-key>/install.journal` per FR-021.
...
**Constraints**: Deterministic output (byte-identical across runs on identical inputs; digests match `install.lock`). Cross-platform correctness with no OS-specific filesystem primitives outside `os.replace`, `os.fsync`, `fcntl.flock`/Win32 `LockFileEx`, and OS-specific pointer primitives selected per platform (`os.symlink`, Windows junction via `mklink /J` or `CreateJunction`). No plaintext secrets in any staged output, journal entry, or lockfile row (Principle I; runtime scan mirrors Spec 007's fail-closed guard). Every publication is atomic-per-file; the final atomic replacement of `.haex-hive/visibility.json` is the sole publication event; readers verify marker + digests before treating a root as available.
...
| III. Project Identity Is Device-Independent (NON-NEGOTIABLE) | PASS | `<repo-key>` in the state root path (FR-021) is a SHA-256 key of the Spec-007 identity (git remote URL or `.harness-id`), never a satellite-specific path. The full identity is retained separately for diagnostics; multiple checkouts of the same repo on one satellite collide safely on the same lock. |
...
| VI. Self-Modifying Instructions Are Always Review-Gated (NON-NEGOTIABLE) | PASS | `haex install` writes device-local state and staged output through the transaction; it does not rewrite `.haex-hive.json`, `constitution.md`, or any versioned config file in place. Per ADR 0010 there is no multi-source constitution merge or `--llm=file` review gate: `haex install` refuses outright (`key=constitution-already-adopted`) when the resolved set carries two or more constitution-contributing molecules, so review-gating is satisfied by refusal rather than an in-tool review step. Schema migrations remain under `haex migrate` (Principle VI v1.3.0). |
...
1. **Atomic per-file publication**: `os.replace(src, dst)` on all three OSes. On Linux/macOS it is `rename(2)` — atomic if same filesystem. On Windows it is `MoveFileExW(..., MOVEFILE_REPLACE_EXISTING)`. No exotic directory-exchange primitive (`renameat2(RENAME_EXCHANGE)`, `renamex_np(RENAME_SWAP)`) is required — the current FR-003 contract selects per-file replacements sequenced by the journal; the final `.haex-hive/visibility.json` write is one `os.replace()` and constitutes the sole publication event (FR-004). Journaled idempotent cleanup follows the publication boundary and never changes the published generation.
...
3. **Mixed-ownership overlay** for `.claude/`, `.codex/`, and future Spec 010 adapter roots:
   ...
   - **Overlay owner-set enumeration** is read from `install.lock`'s per-root `overlay_paths` field and the versioned `ownership.paths` set. The publication step touches exactly those paths and never enumerates siblings, per FR-003.
4. **Fenced-lease numeric contract** (FR-010):
   - **Owner token format**: `<pid>:<hostname>:<start_ns>:<uuid4_hex>` — pid + hostname + monotonic-start-nanoseconds + random UUID4. All fields are ASCII-safe; total length ≤ 128 bytes. Uniqueness is guaranteed by UUID4; the other fields are for operator diagnostics on lock conflict.
   - **Lease record**: `install.mutex` stores the token, immutable UTC `acquired_at`, UTC `heartbeat_at`, reboot-safe `heartbeat_at_ns_wallclock=time.time_ns()`, `heartbeat_interval_ns=5_000_000_000`, `ttl_ns=60_000_000_000`, and `safety_margin_ns=5_000_000_000`. The owner updates and fsyncs the record in place through the locked file handle every 5 seconds; it never replaces the lock pathname or inode.
   - **Revalidation ordering**: recovery first acquires the same non-blocking exclusive OS lock; if a live owner holds it, recovery waits or refuses regardless of age. Once acquired, it requires `time.time_ns() - heartbeat_at_ns_wallclock` greater than TTL plus safety margin, re-reads the record under the lock, verifies unchanged token and wall-clock heartbeat, and rewrites the lease in place to fence the old token before recovery. `mtime` is never sufficient evidence.
5. **Per-root digest scheme** (FR-005):
   ...
   - **`.haex-hive/` root**: digest covers every file under `.haex-hive/` EXCEPT `visibility.json` and `install.lock`; excluding both avoids recursive lock/marker integrity references, per FR-005. The lock's marker reference hashes a canonical marker projection without `install_lock_content_integrity` or `written_at`, so the remaining references are one-way.
   - **Emission format**: `sha256-<base64url(digest)>`, matching Spec 007's SRI-style representation.
6. **Lock primitive selection**: POSIX `fcntl.flock(LOCK_EX | LOCK_NB)` for writers and `LOCK_SH | LOCK_NB` for readers on Linux/macOS; Windows uses `LockFileEx` through `ctypes`, passing `LOCKFILE_EXCLUSIVE_LOCK` for writers and omitting it for readers. Both are non-blocking so lock conflict returns the owner-token payload immediately for the "who has the lock" diagnostic. The Windows lock tests prove that two readers can coexist while a writer is excluded until both release.
7. **Journal replay semantics**: journal is append-only, entries are one JSON-per-line, and each entry's tail hash is SHA-256 over canonical UTF-8 JSON without `tail_hash`, one LF byte, and the previous tail hash as ASCII. On recovery, replay forwards from the last committed marker or roll back to the last known-good marker generation; after a verified marker, recovery performs cleanup only.
...
- **[contracts/install-lock.v2.schema.json](./contracts/install-lock.v2.schema.json)** — extends Spec 007's `install-lock.v2` schema with `overlay_paths` per root, `visibility_marker` block, per-root `content_integrity`, and the versioned per-path `ownership.paths` set.
