...
| VI. Self-Modifying Instructions Are Always Review-Gated (NON-NEGOTIABLE) | PASS | `haex install` writes device-local state and staged output through the transaction; it does not rewrite `.haex-hive.json`, `constitution.md`, or any versioned config file in place. Multi-source constitution merge still flows through the `--llm=file` review gate (now on `haex install` directly since the former `haex constitution assemble` shortcut was retired). Schema migrations remain under `haex migrate` (Principle VI v1.3.0). |
