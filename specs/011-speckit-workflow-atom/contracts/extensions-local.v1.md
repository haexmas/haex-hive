...
2. Remove generated `sources[]` and hook `origin`/atom provenance, plus all atom-owned requirements and hooks (including entries under `.specify/extensions/workflow-atoms/`). Do not copy the generated file wholesale.
3. Adopt the workflow atom (or leave `.haex-hive.json` unchanged if no workflow atom is adopted).
4. Run `haex install`.
5. The regenerated `.specify/extensions.yml` will contain the operator's declarations merged with the workflow atom's fragment (if any).
