...
- **FR-008 (reader resolution)** simplified: after applying Spec 008's FR-005 reader gate, the reader inspects `.haex-hive.json.compounds[]` for a molecule with a non-empty `atoms.workflow` list; found -> that workflow is binding; absent -> the bundled `.specify/workflows/speckit/workflow.yml` is binding. No registry lookup.
...
- **FR-008** (Reader resolution): Before resolving a binding workflow, a reader (an agent, an editor extension, or a validator) MUST apply the Spec 008 FR-005 reader gate: acquire the shared/read lock before the initial `.haex-hive/install.lock` read, retain it through validation and consumption, reject unsupported or retired lock shapes, require every recorded molecule path, require a matching `.specify/.haex-hive-generation.json` record, and require every active adapter pointer to name the lock's `generation_id`. A failed gate makes the installation unavailable and the reader MUST retry after recovery rather than selecting a workflow from an unverified path. Only after that gate passes does it inspect `.haex-hive.json`'s adopted compounds. If exactly one molecule's manifest carries a non-empty `atoms.workflow` list, that molecule's published `workflow.yml` at `.specify/workflows/<molecule-id>/workflow.yml` is binding. If none do, the bundled `.specify/workflows/speckit/workflow.yml` is binding, subject to the same gate. There is no separate selector file to consult. The reader helper's return value carries a `source` field (`molecule` or `bundled`) for diagnostic display.
...
- **SC-002**: The adopted workflow molecule's `constitution.md` fragment appears verbatim under a `### From molecule \`<molecule-id>\` (revision \`<short-sha>\`)` byline inside a shared `## Workflow-Contributed Rules` section in the assembled `.haex-hive/constitution.md` after `haex install --accept-merged`. Verified by a subsection-content check in the US1 integration test.
- **SC-003**: When a workflow molecule is adopted, the reader helper passes the Spec 008 FR-005 reader gate, resolves to the molecule's `workflow.yml`, and returns `source=molecule`; when no workflow molecule is adopted, it passes the same gate, resolves to the bundled `.specify/workflows/speckit/workflow.yml`, and returns `source=bundled`. Verified by unit tests over both cases.
...
- **Spec 007 (Unified Manifest v2)**: baseline atom-manifest schema, `ConsumerManifest`, and `VersionConstraint`; FR-001 uses the current `atoms.workflow`, `atoms.constitution`, `atoms.extensions`, and `atoms.hooks` category fields.
