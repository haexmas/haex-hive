...
**Decision**: when a workflow atom is removed from `.haex-hive.json`, its `.specify/workflows/<atom-id>/` and `.specify/extensions/workflow-atoms/<atom-id>/` directories are removed by the corresponding per-root R1 rename-swap publications (Spec 008 US4 delete-orphans semantics), but only after reconciliation and all relevant validations succeed. `publish_generation` guarantees atomic replacement only for its live directory and the files passed to that call; it does not guarantee a cross-tree commit between `.haex-hive/` and `.specify/`. The constitution fragment ceases to appear in the assembled `.haex-hive/constitution.md` only after successful reconciliation/publication because the atom no longer contributes to the multi-source merge. If `active_workflow` named the removed atom, it resets to `null` only in that successful candidate and emits `key=workflow-atom-reset-to-default`. Any refusal, including `key=no-sources-declared`, preserves the current generation, fragment, registry selection, and atom files. A retry after interruption converges the participating roots.
...
5. Look up `workflows[active_id]`. If missing, or if an atom entry's `atom_id` does not exactly equal its map key, return `fallback` with a diagnostic naming the unresolvable identity. If the selected entry has `source == "bundled"`, require `active_id == "speckit"`; any bundled entry under another key returns `fallback` with the same diagnostic behavior.
