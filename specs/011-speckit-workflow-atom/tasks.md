...
- [ ] T011 Implement `WorkflowAtomManifest` frozen dataclass in [src/haex_hive/workflow/fragment.py](../../src/haex_hive/workflow/fragment.py). `__post_init__` validates every declared path via `RepoRelativePath.validate` + containment against atom root. Fields per [data-model.md §WorkflowAtomManifest](./data-model.md).
- [ ] T012 Implement `ExtensionRequirement` + `HookEntry` frozen dataclasses in [src/haex_hive/workflow/fragment.py](../../src/haex_hive/workflow/fragment.py). `ExtensionRequirement.__post_init__` parses `version_constraint`; raises `InvalidConstraintError` on syntax failure. Depends on T011.
- [ ] T013 Implement `WorkflowFragment` frozen dataclass in [src/haex_hive/workflow/fragment.py](../../src/haex_hive/workflow/fragment.py) plus `load_workflow_fragment(atom_manifest, atom_root) -> WorkflowFragment` loader. Duplicate hook identity or duplicate extension id within one fragment refuses at load. Depends on T011, T012.
- [ ] T014 [P] Implement `LocalExtensionsSource` frozen dataclass in [src/haex_hive/workflow/local_source.py](../../src/haex_hive/workflow/local_source.py) with `load_local_source(consumer_root) -> LocalExtensionsSource`. Record explicit `source_present: bool` alongside `source_bytes: bytes | None`; an absent file returns an empty instance with `source_present=False` and `source_bytes=None`, while a present zero-byte file records `source_present=True` and `source_bytes=b""`. Capture the exact source bytes for later cross-check per data-model.md §Cross-root publication and recovery contract. Depends on T012.
...
- [ ] T017 Implement `InstalledExtensionMetadata` frozen dataclass in [src/haex_hive/workflow/resolver.py](../../src/haex_hive/workflow/resolver.py) with `load_installed_extension_metadata(extensions_root, extension_id) -> InstalledExtensionMetadata` (reads `extension.yml` as authoritative version; no `.registry` cross-check).
- [ ] T018 Implement `WorkflowResolution` frozen dataclass in [src/haex_hive/workflow/resolver.py](../../src/haex_hive/workflow/resolver.py) per data-model.md. Depends on T017.
...
- [ ] T027 [US1] Extend [src/haex_hive/constitution/assemble.py](../../src/haex_hive/constitution/assemble.py) to fold a workflow-atom constitution fragment into the existing merge inside one `## Workflow-Contributed Rules` section under a `### From atom <atom-id> (revision <short-sha>)` byline. Update [src/haex_hive/cli/install.py](../../src/haex_hive/cli/install.py) so a workflow atom's sole constitution contribution is routed through the review-gated workflow-contribution merge instead of `assemble_single_source`; retain the existing shortcut for non-workflow single-source installs. Omit the section when no workflow atom contributes. Add coverage for exactly one constitution contribution. Depends on T020, T022. Satisfies FR-004, FR-010 (reuses existing concealment guard).
...
- [ ] T030 [US1] Wire `cli/install.py::run` to invoke the workflow pipeline: load workflow atom via T020, load fragment via T013, load local source via T014, run `merge_extensions` via T023 in memory, and run the required-extension validation gate via T037 before creating `.specify.next/` or writing any staged output. Only after the merged result validates, create the staging tree, write `.specify.next/extensions.yml`, call `publish_workflow_atom` via T028, and continue through the T027 constitution merge (including the single-contribution routing). Preserve the dependencies and referenced symbols. Depends on T020, T023, T027, T028, T037. Satisfies FR-005 wire-up.

### Integration test for User Story 1

- [ ] T031 [US1] Integration test at [tests/workflow/integration/test_adopt_workflow_atom.py](../../tests/workflow/integration/test_adopt_workflow_atom.py) using a fixture publisher with one `speckit-workflow` atom (with constitution fragment + extensions fragment + hooks): run `haex install --llm=file` + `--accept-merged <candidate>` end-to-end; assert (a) `.specify/workflows/<atom-id>/workflow.yml` byte-identical; (b) hook scripts under `.specify/extensions/workflow-atoms/<atom-id>/`; (c) `.specify/extensions.yml` regenerated with merged content and generated-header comment; (d) `.specify/extensions.local.yml` byte-identical to before (or absent as before); (e) `## Workflow-Contributed Rules` section under correct byline; (f) `resolve_active_workflow` returns `source=atom`; (g) a separate case with exactly one constitution contribution proves the installer uses the workflow-contribution merge path rather than `assemble_single_source`; and (h) workflow generation metadata lists both `.haex-hive/` and `.specify/` as participating roots. Covers SC-001, SC-002, SC-003.
...
- [ ] T035 [P] [US2] Unit test `LocalExtensionsSource` immutability at [tests/workflow/unit/test_local_source.py](../../tests/workflow/unit/test_local_source.py): loading captures `source_present` and `source_bytes` verbatim; absent and present-zero-byte sources are distinct; cover absent-to-empty and empty-to-absent snapshot changes; equal instances compare equal.
...
- [ ] T038 [US2] Wire `cli/install.py::run` to call `validate_required_extensions` (T037) AFTER `merge_extensions` (T023) but BEFORE `.specify.next/` staging. On any raised error, install refuses cleanly without creating staged or temporary output; snapshot existing live managed roots before the attempt and assert that `.specify/` and `.haex-hive/` remain byte-identical. Depends on T030, T037. Satisfies FR-007 wire-up.

### Integration test for User Story 2

- [ ] T039 [US2] Integration test at [tests/workflow/integration/test_required_extension_gate.py](../../tests/workflow/integration/test_required_extension_gate.py): (a) missing extension refuses with `key=required-workflow-extension-missing`; (b) incompatible version refuses with `key=required-workflow-extension-incompatible`; (c) optional missing extension proceeds with warning; (d) invalid version-constraint syntax in atom refuses with `key=invalid-constraint`; (e) atom + local same-id conflict on `homepage` refuses with `key=conflicting-extension-metadata`. For every refusal case, capture pre-install bytes for existing live managed roots and assert they remain byte-identical; separately assert no `.specify.next/`, `.haex-hive.next/`, or other staged/temporary output exists, and that `.specify/extensions.yml` and `.haex-hive/constitution.md` were not rewritten. Covers SC-005.
...
- [ ] T041 [US3] Extend `publish_workflow_atom` (T028) to compute the delete-orphans set as the managed-path difference between the previously adopted workflow atom and the newly adopted workflow atom (or no atom). Include workflow directories, hook directories, constitution sections, and generated extension entries; remove only paths owned by the previous atom, never treating an empty `.specify.next/` tree as permission to erase unrelated consumer-owned `.specify` files. Add an A-to-B integration case that removes atom A's paths while preserving unrelated consumer files. Depends on T028. Satisfies FR-009.
...
- [ ] T046 [US1] Extend the coordinator (T045) with local-source re-read gate: immediately before the first swap, re-read `.specify/extensions.local.yml` from live and compare both its presence and bytes with `LocalExtensionsSource.source_present` and `source_bytes` captured at load time (T014); if either differs, refuse without publishing either root and emit a clear diagnostic. Depends on T014, T045. Satisfies plan.md local-source preservation invariant.
- [ ] T047 [US1] Extend the workflow metadata producers in [src/haex_hive/constitution/assemble.py](../../src/haex_hive/constitution/assemble.py) and the cross-root coordinator so workflow generations record `participating_roots=[".haex-hive/", ".specify/"]` in `visibility.json`, `install.lock`, and `.specify/.haex-hive-generation.json`; preserve the existing `participating_roots=[".haex-hive/"]` form for non-workflow installs. Cover the producer output in T031. Depends on T045. Satisfies the cross-root generation contract.
- [ ] T048 [US1] Extend the coordinator (T045) with two-phase commit order per data-model.md §Cross-root publication and recovery contract: (a) atomically replace or delete `.specify/` managed files in deterministic order excluding `.specify/extensions.local.yml`, saving pre-images to `.specify.prev/`; fsync each changed file plus the `.specify/` directory; (b) commit `.haex-hive/` via Spec 008's `publish_generation` with a `post_write_verify` callback (T049) and `visibility.json` naming both participating roots. Expose a deterministic pause or barrier immediately after the `.specify/` replacement and before the `.haex-hive/` swap so T056 can inspect the mixed-generation interval. Wrap the entire second-root `publish_generation` call, including staging, rename, and fsync failures, in an outer rollback path that restores every changed `.specify/` managed path from `.specify.prev/` after the first swap, then re-raises the original failure; do not roll back before the `.specify/` replacement has occurred. Depends on T045, T046, T047. Satisfies plan.md cross-root contract.
- [ ] T049 [US1] Implement `post_write_verify` callback for the cross-root commit in [src/haex_hive/workflow/publisher.py](../../src/haex_hive/workflow/publisher.py): after `.haex-hive/` swap runs but before `publish_generation` removes `.haex-hive.prev/`, verify (a) both live-root generation records exist; (b) `visibility.json.generation_id` matches `.haex-hive/install.lock.visibility_marker.generation_id` matches `.specify/.haex-hive-generation.json.generation_id`; (c) `participating_roots` lists match byte-identically. On verification failure, restore all changed `.specify/` managed paths from `.specify.prev/` and raise so `publish_generation` restores `.haex-hive/` while its previous root is still available. Depends on T045, T048. Satisfies plan.md verification rollback contract.
- [ ] T050 [US1] Implement recovery under `install/inflight.py` extension in [src/haex_hive/install/inflight.py](../../src/haex_hive/install/inflight.py): under the same exclusive lock as `haex install`, run before any input read on retry. Read and validate `.specify.next/`, `.specify.prev/`, `.haex-hive.next/`, `.haex-hive.prev/` siblings and their generation records. Well-formed and attributably-stale siblings are deleted; malformed or unattributable records refuse recovery and preserve the sibling. After sibling classification, compare live generation records + marker: if only `.specify/` changed, restore from `.specify.prev/`; if `.haex-hive/` also swapped, let Spec 008's mechanism restore from `.haex-hive.prev/`. Fsync every parent directory. Depends on T005. Satisfies plan.md cross-root recovery.
- [ ] T051 [US1] Extend reader helper `resolve_active_workflow` (T022) to enforce the visibility boundary per data-model.md: after loading `.haex-hive/visibility.json`, require its `generation_id` and `participating_roots` to match `.haex-hive/install.lock` AND `.specify/.haex-hive-generation.json`; any mismatch (including the interval between `.specify/` swap and `.haex-hive/` marker swap) rejects the view as unavailable and returns `source=bundled` with a diagnostic. Depends on T022, T005.

### Cross-root recovery + verification-rollback tests

- [ ] T052 [P] [US1] Unit test cross-root generation record round-trip at [tests/workflow/unit/test_cross_root_record.py](../../tests/workflow/unit/test_cross_root_record.py): construct record, serialise, deserialise, verify against schema; malformed records refuse.
- [ ] T053 [US1] Integration test (crash-injection matrix) at [tests/workflow/integration/test_cross_root_recovery.py](../../tests/workflow/integration/test_cross_root_recovery.py): parametrise a crash injection at each of five boundaries per data-model.md §Cross-root publication and recovery contract: (a) after staging siblings; (b) after first `.specify/` managed-path replacement but before `.haex-hive/` swap; (c) after `.haex-hive/` swap but before `post_write_verify`; (d) after `post_write_verify` succeeds; (e) during stale-sibling cleanup. Each case runs the install in a child, kills it at the boundary via `HAEX_HIVE_CRASH_AFTER` seam, then reruns install; asserts convergence to a valid generation and that the live tree matches one of {complete-previous, complete-new}, never mixed.
- [ ] T054 [US1] Integration test verification-rollback at [tests/workflow/integration/test_verification_rollback.py](../../tests/workflow/integration/test_verification_rollback.py): inject a fault where `post_write_verify` deterministically raises AFTER the `.specify/` managed paths are replaced. Assert `.specify/` restored byte-for-byte from `.specify.prev/`, `.haex-hive/` restored by Spec 008's mechanism, both `.next/` and `.prev/` siblings cleaned per recovery contract, and the final tree matches the pre-install state exactly (including `.specify/extensions.local.yml` untouched).
- [ ] T055 [US1] Integration test local-source preservation invariant at [tests/workflow/integration/test_local_source_preserved.py](../../tests/workflow/integration/test_local_source_preserved.py): six scenarios asserting `.specify/extensions.local.yml` byte-identity: (a) successful install with atom adopted; (b) install with atom removed (downgrade); (c) install refused at required-extension gate; (d) install refused at multi-workflow-atoms; (e) install crashed after `.specify/` replacement and recovered; (f) install with `.specify/extensions.local.yml` absent (empty declarations, install succeeds, file remains absent). Include unit coverage for absent-to-empty and empty-to-absent source transitions in T035.
- [ ] T056 [US1] Integration test mixed-generation reader-rejection at [tests/workflow/integration/test_mixed_generation_reader.py](../../tests/workflow/integration/test_mixed_generation_reader.py): use the deterministic pause/barrier from T048 immediately after the `.specify/` managed-path swap and before the `.haex-hive/` swap, then invoke `resolve_active_workflow` while the writer is paused. Assert the reader helper (T051) returns `source=bundled` with a diagnostic rather than exposing a mixed view; do not rely on a timing race.

---

## Phase 8: Polish & Cross-Cutting Concerns

- [ ] T057 [P] Dead-symbol prune sweep: run `ruff check --select F401,F841 src tests` after all implementation lands; remove any transient imports the workflow subpackage or extended modules no longer need. Match Spec 008 T055 discipline.
- [ ] T058 [P] Refresh public surface in [src/haex_hive/workflow/__init__.py](../../src/haex_hive/workflow/__init__.py): re-export `WorkflowAtomManifest`, `WorkflowFragment`, `LocalExtensionsSource`, `GeneratedExtensionsYml`, `WorkflowResolution`, `resolve_active_workflow`; keep everything else module-private.
- [ ] T059 Run [specs/011-speckit-workflow-atom/quickstart.md](./quickstart.md) end-to-end on a fresh clone with `$HAEX_HIVE_STATE` pointing at a scratch dir: exercise all 9 steps (adopt, first install with local source, verify publication, verify binding, refusal path missing extension, refusal path two workflows, downgrade, where-things-live). Record SC-001..SC-006 verification outcomes in the phase closing commit message.
- [ ] T060 Full pytest run (`pytest tests/workflow/` + full-suite regression) after all implementation lands. Zero regressions; every new test added by Phases 3-7 passes.
- [ ] T061 Constitution v1.4.1 follow-up: after Spec 011 lands, propose a PATCH-level amendment to [.specify/memory/constitution.md](../../.specify/memory/constitution.md) that retires the "planned Spec 011" forward-reference in the § Development Workflow declared-speckit-workflow-adherence bullet. Task-slot retained for traceability; the amendment is a separate PR.

### Deferred slots

- [ ] T062 [deferred] Runtime enforcement of declared workflow adherence (pre-commit hook or CI gate refusing task landings that skip declared workflow steps). Deferred to a Phase-7 constitution amendment per plan.md §Complexity Tracking. Not part of Spec 011.
- [ ] T063 [deferred] Automatic installation of speckit-community extensions when `required_extensions[]` are absent locally. Delegated to specifyr; out of scope per spec.md §Assumptions.
- [ ] T064 [blocked on T037 (Spec 008)] `haex install --verify-only` integration: report the resolved `WorkflowResolution` in verify-only output. Requires Spec 008 T037 (`--verify-only` + shared-read lock) to land first. Task-slot retained.
...
- **Cross-Root Recovery (Phase 7)**: US1 pipeline must be in place first; T045-T051 land during or shortly after Phase 3. Recovery tests T052-T056 depend on the full Phase 7 implementation.
- **Polish (Phase 8)**: depends on all user stories + Phase 7 complete.

### Parallel Opportunities

- All contract tests (T006-T010) run in parallel.
- Dataclass tasks sharing `fragment.py` (T011-T013) run sequentially; T013 still depends on T011 and T012. T014 can run after T012, T015 and T016 can run independently, and resolver dataclass tasks T017-T018 run sequentially because they share `resolver.py`.
...
# Dataclass implementations by file:
Task: "WorkflowAtomManifest at src/haex_hive/workflow/fragment.py"  (T011)
Task: "ExtensionRequirement + HookEntry at src/haex_hive/workflow/fragment.py"  (T012, after T011)
Task: "WorkflowFragment loader at src/haex_hive/workflow/fragment.py"  (T013, after T011+T012)
Task: "LocalExtensionsSource at src/haex_hive/workflow/local_source.py"  (T014, after T012; can run with T015-T016)
...
Task: "WorkflowResolution at src/haex_hive/workflow/resolver.py"  (T018, after T017)
...
| FR-002 (publication location) | T028, T030, T045, T047, T048 | T031 |
| FR-003 (hook scripts + reserved namespace + collisions) | T028, T041 | T031, T042 |
| FR-004 (constitution fragment merge) | T027 | T031 |
| FR-005 (extensions fragment merge + local ownership) | T013, T014, T016, T021, T023, T030 | T007, T008, T009, T025, T033, T034, T035, T039, T055 |
| FR-006 (multi-workflow-atom refusal) | T020, T019 | T043, T044 |
| FR-007 (required-extensions gate) | T017, T037, T038 | T036, T039 |
| FR-008 (reader resolution) | T018, T022, T051 | T026, T056 |
...
| SC-004 (delete-orphans) | T042, T055 (b) |
...
|---|---|---|
| Cross-root publication recovery | T045, T047, T048, T050, T051 | T052, T053, T056 |
| Local-source preservation invariant | T014 (captures presence + source_bytes), T023 (never writes to local), T028 (excludes local from staging), T046 (re-read gate) | T035, T055 |
| Verification rollback | T049 (`post_write_verify` callback), T048 (`.specify.prev/` staging + outer rollback), T050 (recovery) | T054 |
