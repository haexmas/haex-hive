...
- [ ] T010 [P] Implement `WorkflowRegistry` + `WorkflowEntry` frozen dataclasses in [src/haex_hive/workflow/registry.py](../../src/haex_hive/workflow/registry.py) with `from_json(raw: bytes)` (validates via `workflow-registry.v1.schema.json`) and `to_json_bytes()` (deterministic serialisation: sorted keys, 2-space indent, LF newlines). Depends on T005, T009.
...
- [ ] T022 [US1] Implement `publish_workflow_atoms(atoms, dest_root, transaction)` in [src/haex_hive/workflow/registry.py](../../src/haex_hive/workflow/registry.py): copies each atom's `workflow.yml` payload to `.specify/workflows/<atom-id>/workflow.yml`; copies its `hooks_dir` tree (if any) to `.specify/extensions/workflow-atoms/<atom-id>/`. All destination paths validated for containment against consumer root. Publication participates in the same rename-swap generation as the constitution merge. Depends on T015. Satisfies FR-002 and FR-003.
...
- [ ] T024 [US1] Wire `cli/install.py::run` to call `publish_workflow_atoms` (T022), fold workflow-atom constitution fragments (T021 already integrated in assemble), and rewrite `workflow-registry.json` via `WorkflowRegistry.upsert` (T023). Depends on T021, T022, T023. Satisfies FR-002, FR-003, FR-004, FR-006.

### Integration test for User Story 1

- [ ] T025 [US1] Integration test at [tests/workflow/integration/test_adopt_workflow_atom.py](../../tests/workflow/integration/test_adopt_workflow_atom.py) using a fixture publisher repo declaring one `speckit-workflow` atom: run `haex install --llm=file` + `--accept-merged <candidate>` end-to-end; assert (a) workflow.yml published byte-for-byte; (b) hook scripts published under `.specify/extensions/workflow-atoms/<atom-id>/`; (c) constitution fragment merged into `## Workflow-Contributed Rules` with the correct byline; (d) `workflow-registry.json` lists both workflows; (e) setting `active_workflow` in the registry and calling `resolve_active_workflow` returns the atom-workflow. Covers SC-001, SC-002, SC-003.
...
- [ ] T029 [US2] Implement `validate_required_extensions(resolved: Iterable[ResolvedExtensionRequirement], extensions_root: Path) -> None` in [src/haex_hive/workflow/resolver.py](../../src/haex_hive/workflow/resolver.py): raises `RequiredWorkflowExtensionMissingError` when `.specify/extensions/<id>/` is absent for a required id; raises `RequiredWorkflowExtensionIncompatibleError` when installed version fails the constraint; emits `OptionalWorkflowExtensionConflictWarning` on stderr for compatible-optional cases and returns cleanly. Depends on T012, T014. Satisfies FR-007.
...
- [ ] T032 [US2] Integration test at [tests/workflow/integration/test_required_extension_gate.py](../../tests/workflow/integration/test_required_extension_gate.py). Four subtests: (a) missing extension refuses with `key=required-workflow-extension-missing` and no files written; (b) incompatible version refuses with `key=required-workflow-extension-incompatible` and stderr names both installed and required versions; (c) optional missing extension proceeds with a stderr warning; (d) two atoms declaring the same extension with compatible constraints merge to the canonical form. Covers SC-005.
...
- [ ] T036 [US3] Integration test at [tests/workflow/integration/test_delete_orphans_workflow_atom.py](../../tests/workflow/integration/test_delete_orphans_workflow_atom.py): reuse fixture from T025. Three subtests: (a) remove atom while `active_workflow` names it, verify auto-reset + `key=workflow-atom-reset-to-default` on stderr; (b) remove atom while `active_workflow` names something else, verify preservation; (c) verify `.specify/workflows/<atom-id>/` and `.specify/extensions/workflow-atoms/<atom-id>/` cleanup atomically via Spec 008 rename-swap (assert absence after successful install). Covers SC-004.
...
- Data-model + small helper tasks (T009-T014) are all `[P]` and complete before their story tasks consume them.

### Parallel Opportunities

- All contract tests (T006, T007, T008) run in parallel.
- All dataclass implementations (T009, T010, T011, T012, T013) run in parallel (each in its own file / class).
...
Task: "WorkflowRegistry+WorkflowEntry at src/haex_hive/workflow/registry.py"  (T010)
...
# T021 (assemble extension) → T022 (publish_workflow_atoms) → T023 (upsert)
...
|---|---|---|
| FR-001 (workflow atom shape + path safety) | T009, T015 | T007, T018 |
| FR-002 (publication location) | T022, T024 | T025 |
| FR-003 (hook scripts) | T022 | T025 |
| FR-004 (constitution fragment merge) | T021 | T025 |
| FR-005 (extensions fragment merge) | T011, T016, T030 | T008, T028, T032 |
...
| FR-010 (concealment guard on fragments) | T021 (reuses existing `validate_no_concealment_instructions`) | T025 asserts no publication when fragment carries concealment content |
...
| `optional-workflow-extension-conflict` | T029 (warning-only, stderr) | T032 (c) |
