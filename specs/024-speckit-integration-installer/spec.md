...
- **FR-006**: In a non-interactive invocation, `spaex install` MUST NOT prompt. It MUST use an explicit concrete command-line selection or a previously persisted selection; `all` MUST NOT count as a safe explicit selection and MUST refuse without interactive confirmation.
- **FR-006a**: The `all` shorthand MUST only take effect after the interactive selector has displayed the available integrations and the operator has entered `all`; no installer, recipe, molecule README, or default may silently choose every declared integration.
