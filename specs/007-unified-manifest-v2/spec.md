...
- **Molecule declaring `atoms: {}` (empty map)**: refused during schema validation. Every molecule requires at least one category populated; the schema enforces this via `minProperties: 1` on `atoms`.
...
- **Category**: a named slot in the `atoms` map. Each admitted category is owned by exactly one spec that defines its cardinality (`workflow`, `extensions` are cardinality-one; `hooks`, `constitution` accept 0..N) and its publication contract.
