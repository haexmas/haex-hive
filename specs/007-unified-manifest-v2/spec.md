...
1. **Wrong noun**: what Spec 007 v2 called an "atom" is structurally a **molecule**: a directory containing a manifest plus multiple bonded artifacts. The **individual delivered files** (a `workflow.yml`, a `constitution.md`, a single hook script, a single skill markdown) are the atoms in the chemistry metaphor. And the consumer-side adoption entry (`source` + `revision` + a list of molecule references) is a **compound**: several molecules bound together at one source pin. Under this reframe three levels are named:
   ...
   - **Atom**: a single delivered artifact inside a molecule, listed in `atoms.<category>[]`.
...
- **`contributes.<category>: <path>`** mechanism on the molecule manifest: retired. Replaced by `atoms` (see FR-001).
...
- **`atoms` field** on the molecule manifest: required, shape `dict[str, list[str]]`. Maps a category name (e.g. `constitution`, `workflow`, `hooks`, `extensions`) to a list of repo-relative paths inside the molecule directory. Every declared path passes `RepoRelativePath.validate` and canonical containment against the molecule root at manifest-load time. Every path MUST resolve to a regular file after canonicalisation. Which categories a molecule populates fully identifies what the molecule ships; no separate discriminator field exists. Each category owns its cardinality and publication semantics (see FR-003), documented in the spec that introduces the category.
...
- **New diagnostic keys** (reuse existing exit-code categories):
  ...
  - `atoms-path-escape` (`INPUT_REFUSE=2`)
  - `atoms-path-duplicate` (`INPUT_REFUSE=2`)
  - `atoms-category-overlap` (`INPUT_REFUSE=2`)
  - `atoms-cardinality-violation` (`INPUT_REFUSE=2`)
  - `atoms-target-invalid` (`INPUT_REFUSE=2`)
  - `unknown-atoms-category` (`INPUT_REFUSE=2`)
  - `molecule-id-duplicate` (`INPUT_REFUSE=2`)
  - `molecule-id-revision-conflict` (`INPUT_REFUSE=2`)
...
- Every existing molecule's committed content survives byte-identically; only its manifest.json fields change (`contributes` → `atoms`).

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Publisher authors and consumer adopts a molecule under v3 (Priority: P1) MVP

A publisher authors a molecule directory containing `manifest.json` (declaring `atoms`, listed inside a publisher-root `manifest.json` that lists the molecule), a constitution fragment file, and any additional atoms per the delivered categories. A consumer adds a compound to `.haex-hive.json.compounds[]` naming the source URL, the full 40-char SHA revision, and the molecule-id to fetch. Running `haex install` publishes every atom the molecule's `atoms` map names into its category-specific publication targets. The rename atom→molecule and the new compound layer surface in every diagnostic, log line, and file the operator interacts with.
...
**Independent Test**: On a fresh publisher checkout, create a molecule at `test-constitution/` with `manifest.json` declaring `atoms: { constitution: ["constitution.md"] }` and a `constitution.md` file. Register it in the publisher-root `manifest.json.molecules`. Clone into a state-root. On a consumer checkout, add `compounds: [{ source, revision, molecules: ["<molecule-id>"] }]` to `.haex-hive.json`. Run `haex install`. Verify (a) publisher's molecule manifest validates against `molecule-manifest.v3.schema.json`; (b) consumer's `.haex-hive.json` validates against the updated consumer manifest schema; (c) `haex install` completes; (d) `.haex-hive/constitution.md` contains the delivered atom's content.

**Acceptance Scenarios**:

1. **Given** a publisher molecule with `atoms: { constitution: ["constitution.md"] }` and a valid `constitution.md`, **When** a consumer adopts it via a compound and runs `haex install`, **Then** the atom's content appears in `.haex-hive/constitution.md` as a byte-for-byte copy (ADR 0010: a repository adopts exactly one constitution-contributing molecule; `haex install` refuses with `key=constitution-already-adopted` if the resolved set carries a second one), and `install.lock.molecules[]` records the molecule's `(id, source, revision, contributed_paths)`.
2. **Given** a publisher molecule with a multi-artifact `atoms` (workflow + 1 constitution fragment + 2 hooks), **When** the consumer adopts it via a compound, **Then** every atom named in `atoms` publishes into its category-specific target (per Spec 011); cardinality-one categories (`workflow`) refuse if the list has 0 or 2+ entries.
...
### User Story 2 - Atom paths are safety-checked (Priority: P2)

A publisher accidentally or maliciously ships a molecule whose `atoms` map contains a path that escapes the molecule root (absolute, `.`/`..` traversal, symlink escape), duplicates a path across categories, or violates a category's cardinality. `haex install` refuses before any file is published, naming the specific safety violation.
...
1. **Given** a molecule whose `atoms.constitution` contains `"../evil.md"`, **When** `haex install` loads its manifest, **Then** the install refuses with `key=atoms-path-escape` and stderr names the path and cites Principle II. Zero files are published.
2. **Given** a molecule whose `atoms.hooks` list contains `"hooks/a.sh"` twice, **When** `haex install` loads its manifest, **Then** the install refuses with `key=atoms-path-duplicate` and stderr names the offending category and path.
3. **Given** a molecule whose `atoms.extensions` and `atoms.hooks` both list `"shared/foo.sh"`, **When** `haex install` loads its manifest, **Then** the install refuses with `key=atoms-category-overlap` and stderr names the shared path and both categories.
4. **Given** a molecule with `atoms.workflow` list having zero entries (or two entries), **When** `haex install` loads its manifest, **Then** the install refuses with `key=atoms-cardinality-violation` and stderr names the category and expected cardinality.
5. **Given** a molecule whose `atoms.constitution` names a missing path or a directory, **When** `haex install` loads its manifest, **Then** it refuses with `key=atoms-target-invalid` before staging.
6. **Given** a molecule declaring `atoms.wibble: [...]` (category not owned by any spec), **When** `haex install` loads its manifest, **Then** it refuses with `key=unknown-atoms-category` naming the offending category. The list of admitted categories at v3 landing is `{constitution, workflow, extensions, hooks}`; downstream specs may add new categories only together with their publication contract.

### User Story 3 - Spec 011 workflow molecules work under v3 without bespoke fields (Priority: P2)

A workflow-molecule author (Spec 011) puts every delivered artifact (workflow.yml, constitution fragment, extensions fragment, hooks) into `atoms`. No bespoke `contributes.speckit_workflow` / `contributes.speckit_extensions` / `contributes.speckit_hooks` fields exist; a workflow molecule is any molecule whose `atoms.workflow` list is populated.
...
**Independent Test**: Author a molecule with `atoms.workflow: ["workflow.yml"]`, `atoms.constitution: ["constitution.md"]`, `atoms.extensions: ["extensions.yml"]`, `atoms.hooks: ["hooks/pre.sh", "hooks/post.sh"]`. Adopt on a consumer via a compound. Assert the Spec 011 publication semantics apply verbatim (workflow.yml under `.specify/workflows/<molecule-id>/`, hooks under reserved namespace, etc.) with no bespoke fields consulted.

**Acceptance Scenarios**:

1. **Given** a workflow molecule (non-empty `atoms.workflow`) adopted via a compound under v3, **When** `haex install` publishes it (per Spec 011 semantics), **Then** every atom named in `atoms` publishes to the same target that Spec 011 v1 published to when using the retired `contributes.speckit_*` fields.
2. **Given** two adopted molecules both populating `atoms.workflow` referenced from any compounds in a single `.haex-hive.json.compounds[]`, **When** `haex install` runs, **Then** it refuses per Spec 011 FR-006 (`key=multiple-workflow-molecules-refused`). This v3 amendment does not change Spec 011's per-category uniqueness invariants; it only replaces the discovery mechanism.

### User Story 4 - Migration of existing molecules (Priority: P3)

The two haex-hive-self molecules (`com.github.haexmas.haex-hive.constitution`, `com.github.haexmas.haex-hive.graphify-first-authoring`) migrate their `manifest.json` from `contributes.constitution: "constitution.md"` to `atoms: { constitution: ["constitution.md"] }`. Both molecule manifest versions bump MAJOR because the field schema is a breaking change.
...
- **Molecule declaring `atoms: {}` (empty map)**: refused during schema validation. Every molecule requires at least one category key and at least one delivered path across those categories; the schema enforces `minProperties: 1` on `atoms`, and semantic validation rejects an all-empty category map.
- **Molecule declaring an unknown category** (e.g. `atoms.wibble: ["something.yaml"]`): refused with `key=unknown-atoms-category` naming the offending category. Categories are admitted by the spec that owns their publication contract; the initial admitted set at v3 landing is `{constitution, workflow, extensions, hooks}`.
- **Molecule populating `atoms.workflow` with zero entries (empty list)**: refused with `key=atoms-cardinality-violation` (`workflow` is cardinality-one per Spec 011).
- **Directory, missing path, or other non-file target listed in `atoms`**: refused with `key=atoms-target-invalid` before staging. Categories that explicitly accept directory targets in a future spec must define that exception in their per-category contract.
- **Duplicate molecule references in `.haex-hive.json.compounds[]`**: refused with `key=molecule-id-duplicate`, whether repeated in one compound or across compounds. The same ID from one source at different revisions is refused with `key=molecule-id-revision-conflict`; the resolver never chooses a revision implicitly. The existing cross-source collision remains `key=molecule-id-collision`.
...
- **FR-001** (Molecule manifest schema): The molecule manifest at `<molecule-dir>/manifest.json` MUST match `molecule-manifest.v3.schema.json`. Required fields: `haex_hive_version: "3"`, `id` (reverse-DNS), `version` (SemVer 2.0.0), `priority` (integer), `atoms` (object mapping category name to a list of repo-relative paths, `minProperties: 1`). Optional fields: any that Spec 007 v2 already accepted at the top level and remain applicable. The retired `contributes` field MUST NOT be present; presence refuses with `key=molecule-manifest-schema-invalid`. A v2-to-v3 migration MUST add `priority: 100` when the source omits it and MUST preserve any existing priority.
- **FR-002** (atoms path safety): Every path in `atoms.<category>[i]` MUST pass `RepoRelativePath.validate` and a canonical containment check against the molecule root at manifest-load time. After resolution, every target MUST exist and be a regular file; missing paths, directories, symlinks or reparse points that do not resolve to regular files, and other non-file targets MUST refuse with `key=atoms-target-invalid`. Absolute paths, backslash- or drive-qualified paths, `.`/`..` traversal, and symlink or reparse-point targets that escape the molecule root MUST refuse with `key=atoms-path-escape` citing Principle II. All refusals MUST fire before any file is written to consumer roots.
- **FR-003** (atoms uniqueness and cardinality invariants): (a) No path appears more than once within a single `atoms.<category>` list; violation refuses with `key=atoms-path-duplicate`. (b) No path appears in more than one category within the same `atoms` map; violation refuses with `key=atoms-category-overlap`. (c) At least one path MUST be delivered across all category lists; a molecule whose categories are present but all lists are empty refuses with `key=atoms-cardinality-violation`. Individual empty lists remain valid where their category contract permits them. (d) Each category has a cardinality rule owned by the spec that introduces it. Cardinality-one categories (`workflow`, `extensions` per Spec 011) refuse with `key=atoms-cardinality-violation` when the list has 0 or 2+ entries; multi-file categories (`hooks`, `constitution`) accept 0..N entries. Per-category cardinality rules are documented in each category's owning spec.
- **FR-004** (admitted category set): The admitted `atoms` category set at v3 landing is `{constitution, workflow, extensions, hooks}`. Spec 007 owns the molecule-manifest category contract and path-safety rules for all four categories; Spec 008 owns the constitution publication implementation and transaction integration, while Spec 011 owns the workflow, extensions, and hooks publication implementations. A molecule declaring a category not in the admitted set MUST refuse with `key=unknown-atoms-category` naming the offending category. A downstream spec MAY add a new category only when it defines that category's cardinality, containment target, and `haex install` dispatch; adding a category does NOT require re-versioning `molecule-manifest.v3.schema.json`.
- **FR-005** (consumer manifest compound layer): `.haex-hive.json` MUST use the top-level key `compounds` (list of `{source, revision, molecules[]}`), NOT the retired `atoms`. Consumers using `atoms` at the top level refuse at manifest-load with `key=molecule-manifest-schema-invalid`. Every entry's `revision` MUST be a full 40-char SHA per Principle IV. Every entry's `molecules[]` MUST be a non-empty list of unique reverse-DNS molecule-ids. A molecule ID MUST occur at most once across the complete `compounds[]` list; duplicates, including duplicates inside one list, refuse with `key=molecule-id-duplicate`. The same ID from one source at different revisions refuses deterministically with `key=molecule-id-revision-conflict`; no revision-precedence rule exists. The v2-to-v3 migration MUST rename the outer list from `atoms` to `compounds` and each entry's `includes[]` to `molecules[]`, then bump `haex_hive_version` to `"3"`.
...
- **FR-008** (haex-hive-self molecule migration): The two haex-hive-self molecules (`com.github.haexmas.haex-hive.constitution` at `.specify/memory/` and `com.github.haexmas.haex-hive.graphify-first-authoring` at `.specify/molecules/graphify-first-authoring/`) MUST migrate their manifest.json in this amendment landing to `atoms: { constitution: ["constitution.md"] }`. Both version fields bump MAJOR. The publisher-root `manifest.json` at the repo root MUST rename its top-level `atoms` key to `molecules`. The consumer-side `.haex-hive.json` at the repo root MUST rename its top-level `atoms` key to `compounds` and each entry's `includes` to `molecules`.
...
- **Molecule** (packaging unit): a directory containing `manifest.json` (declaring `id`, `version`, `priority`, `atoms`) plus the atoms named in `atoms.*[]`. Lives in a publisher repository. Formerly called "atom" in Spec 007 v2.
- **Atom** (individual delivered artifact): a single file inside a molecule's directory that the molecule's `atoms.<category>[]` list names. Examples: a `constitution.md`, a `workflow.yml`, a single `hook.sh`, a single `skill.md`. Not manifest-level; conceptual.
- **Atoms map**: the `atoms` field on a molecule manifest. Maps a category name (`constitution`, `workflow`, `hooks`, `extensions`, ...) to a list of repo-relative atom paths. Which categories a molecule populates fully identifies what the molecule ships; no separate discriminator field exists.
- **Category**: a named slot in the `atoms` map. Spec 007 owns the shared category schema and invariants; the owning publication spec defines dispatch and category-specific cardinality (`workflow`, `extensions` are cardinality-one; `hooks`, `constitution` accept 0..N). Constitution publication is implemented by Spec 008 under Spec 007's contract; workflow, extensions, and hooks publication are implemented by Spec 011 under the same contract.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A publisher molecule with `atoms: { constitution: ["constitution.md"] }` validates against `molecule-manifest.v3.schema.json` and adopts cleanly on a fresh consumer via a compound. Verified by an integration test using the migrated haex-hive-self constitution molecule.
- **SC-002**: A publisher molecule with any of the atom-safety violations (path escape, duplicate, cross-category overlap, cardinality violation, invalid target, unknown category) refuses `haex install` with the documented diagnostic key and zero files written under any managed root. Verified by six separate refusal integration tests (US2 acceptance scenarios 1-6).
- **SC-003**: The haex-hive-self checkout at the amendment-landing commit has both molecule manifests migrated to `atoms`, and `haex install` on a scratch consumer that references them via a compound succeeds and produces a byte-identical `.haex-hive/constitution.md` compared to the pre-amendment output. Verified by a self-adopt regression test.
...
- **SC-005**: The Spec 011 workflow mechanism works under v3 without any bespoke `contributes.speckit_*` field: the same workflow.yml + constitution + extensions + hooks publish to their Spec 011 targets when adopted as a v3 molecule (via a compound) with `atoms.workflow` populated. Verified by an integration test.
...
- **Spec 011** (Speckit workflow molecule, in draft on branch): the previously-drafted spec 011 introduced bespoke `contributes.speckit_*` fields. Under this amendment those become unnecessary; Spec 011 rebuilds on the v3 fundament with `atoms.workflow` populated. Spec 011's draft artifacts (currently unmerged on any branch after PR-56 was merged then abandoned via this amendment) will be re-drafted in a follow-up cycle after v3 lands.
