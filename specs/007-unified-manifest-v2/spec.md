...
1. **Given** a publisher molecule with `atoms: { constitution: ["constitution.md"] }` and a valid `constitution.md`, **When** a consumer adopts it via a compound and runs `haex install`, **Then** the atom's content appears in `.haex-hive/constitution.md` as a byte-for-byte copy (ADR 0010: a repository adopts exactly one constitution-contributing molecule; `haex install` refuses with `key=constitution-already-adopted` if the resolved set carries a second one), and `install.lock.molecules[]` records the molecule's `(id, source, revision, paths[])`.
