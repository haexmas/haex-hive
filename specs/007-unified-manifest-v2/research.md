...
**Decision**: Deferred to Spec 007's implementation phase as a **plugin-style boundary interface**, not resolved here. The Spec 007 CLI defines an internal `haex_hive.constitution.llm.MergeLLM` protocol: `merge(contributions: Sequence[ResolvedConstitutionContribution], task_prompt: str) -> MergeResult`, where every contribution carries both its `ConstitutionSource` metadata and exact raw body. `MergeResult` contains `candidate: bytes` and `confirmed: bool`; orchestration MUST publish only a `confirmed=True` result. The `stdio` adapter must display every contribution with its source metadata and body, then read a candidate record of ASCII `Content-Length: <N>\n` and exactly `N` UTF-8 candidate bytes. It terminal-validates and displays the candidate, prompts for confirmation, then reads the separate exact ASCII record `--haex-confirm: yes\n`. Any EOF, malformed length/record, or other confirmation returns `confirmed=False`; the controller refuses with `key=merge-not-confirmed` and changes no output. Three registered implementations are selected at command-time via a `--llm=<method>` flag or the `HAEX_LLM` environment variable (in the design's order of precedence: `--llm` > env > auto-detect):

- **`stdio`**: after the Principle-I and terminal-display guards accept the content, displays every source plus a natural-language prompt to stdout and reads the merged result from stdin (terminates on EOF). It requires an operator-attached LLM in the local interactive session. The default for `--llm=stdio` and the default when `HAEX_LLM=stdio`.
...
`--accept-merged <path>` selects phase two of the `file` flow before any `--llm`, environment, or TTY auto-detection. It is mutually exclusive with `--llm`, so the documented phase-two invocation behaves identically in TTY and non-TTY environments.

Before a resolved contribution is copied, passed to `MergeLLM`, or serialized into pending merge state, `haex_hive.constitution.safety.validate_no_plaintext_secrets(content)` MUST enforce Principle I. It also validates each final candidate and every serialized pending or lock payload before writing. The guard rejects private-key blocks, recognized provider-token prefixes, credentials in URL-like values, and non-redacted `password`, `secret`, `token`, or `api_key` assignments; it emits only `key=plaintext-secret-detected` and a non-sensitive location, never the matching value. A match has no override: no source is displayed to `stdio`, no pending file or journal is written, and no output target is modified. False positives fail closed for operator remediation. Before `stdio` displays a source or final candidate, `validate_terminal_safe_display` MUST reject escape, carriage-return, backspace, bidirectional, and every other non-allowlisted control character (only LF and TAB are allowed controls); it returns `key=terminal-unsafe-contribution` without printing that body. Before a confirmed `MergeResult` or any `--accept-merged` candidate can be staged, `validate_no_concealment_instructions(candidate)` also enforces Principle VIII. It refuses a candidate containing invisible or bidirectional control characters, hidden markup, or an instruction to a downstream agent to conceal or withhold relevant information from the operator. Its machine-readable refusal is `key=constitution-concealment-instruction`; the candidate is not staged, no publication journal is written, and both output targets remain unchanged. Human review and confirmation are additional safeguards, not a bypass. Unit tests cover each secret, terminal-control, and concealment prohibited form plus ordinary visible content; integration tests cover source, `stdio`, and `file`/`--accept-merged` refusal before any pending or publication write.
