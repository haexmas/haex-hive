...
Before any result from `MergeLLM.merge(...)` or any `--accept-merged` candidate can be staged, `haex_hive.constitution.safety.validate_no_concealment_instructions(candidate)` MUST enforce Principle VIII. It refuses a candidate containing invisible or bidirectional control characters, hidden markup, or an instruction to a downstream agent to conceal or withhold relevant information from the operator. The machine-readable refusal is `key=constitution-concealment-instruction`; the candidate is not staged, no publication journal is written, and both output targets remain unchanged. Human review and confirmation are additional safeguards, not a bypass. Unit tests cover each prohibited form and an ordinary visible constitution; integration tests cover `stdio` and `file`/`--accept-merged` refusal before publication.
