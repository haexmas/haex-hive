...
- **Atom manifests at pinned SHAs**: read via `git show <sha>:<publisher_atom_entry.path>/manifest.json` and validated against `atom-manifest.v2.schema.json`. Only manifests declaring `contributes.constitution` contribute to this command; a selected atom without that declaration is ignored for constitution assembly.
...
|---|---|---|
| 0 | Success | |
| 2 | Resolution refuse — zero constitution sources or one or more manifests could not be resolved | A selected atom without `contributes.constitution` is a non-contribution, not an error; when that leaves zero resolved contributions the command emits `key=no-sources-declared` with message `no constitution sources declared`. Other resolution failures carry specific keys, including `key=publisher-manifest-not-found`, `key=atom-manifest-not-found`, and `key=atom-id-collision`. Existing output files are untouched. See FR-025. |
| 3 | I/O refuse — required content is unavailable in the local publisher clone | Use `key=publisher-clone-unavailable` when the clone is absent or unreadable, `key=pinned-revision-not-found` when the pinned SHA is absent, and `key=contribution-file-not-found` when an atom that declares `contributes.constitution` names a contribution file absent at that pinned SHA. See spec Edge Cases. |
