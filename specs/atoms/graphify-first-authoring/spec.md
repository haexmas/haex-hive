...
- **FR-011**: The atom's installer MUST verify the `graphify` CLI is present on PATH and refuse with actionable instructions if it is not, making no other changes.
- **FR-012**: The atom's installer MUST ask for confirmation before invoking `graphify install` on the operator's behalf; it MUST NOT silently install the `graphify` package itself into the operator's Python environment.
...
- The `graphify` CLI (package `graphifyy`) is a prerequisite the operator installs separately (`pip install graphifyy`); this atom does not install it, only verifies its presence.
