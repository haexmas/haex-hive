...
- **FR-011**: The atom's installer MUST verify the `graphify` CLI is present on PATH. If absent, it MUST prompt the operator to install it via `pip install graphifyy` (default Yes); on decline, the installer MUST refuse with actionable instructions, making no other changes.
- **FR-012**: The atom's installer MUST invoke `graphify install` on the operator's behalf only when the repo has no `graphify-out/` directory (heuristic that graphify has not yet been used here); when the directory already exists, `graphify install` MUST be skipped. Installation of the `graphify` package itself (`graphifyy`) is governed by FR-011's prompt; the installer MUST NOT silently install it into the operator's Python environment outside that prompt.
