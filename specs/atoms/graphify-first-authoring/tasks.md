...
- [ ] T008 [US1] Manually verify via [quickstart.md](quickstart.md) steps 3–5 (adopt in `.haex-hive.json`, `haex constitution assemble`, `haex constitution show`) that the contributed text is correctly picked up — this is a behavioral check, not a unit test — deferred to Phase 7 (covered by T024/T025 which run `haex constitution assemble` and `show` on this repo's own self-adoption)
...
- [X] T009 [P] [US2] Implement refresh logic in `.specify/atoms/graphify-first-authoring/hooks/_refresh.py`: invoke `graphify <repo-root> --update`; on failure, warn to stderr and return normally rather than raising (contracts/git-hooks.md, FR-006)
...
- [X] T011 [P] [US2] Unit tests for refresh logic in `tests/atoms/graphify_first_authoring/test_refresh.py`: successful refresh updates the freshness marker, simulated `graphify` failure warns without raising (depends on T009)
- [X] T012 [US2] Integration test in `tests/atoms/graphify_first_authoring/test_post_commit_hook.py`: real `git commit` on a tracked branch in a scratch repo triggers the installed hook and `graphify-out/.meta.json`'s `indexed_at_sha` matches the new `HEAD` (depends on T010)
...
- [X] T018 [US4] Implement installer preconditions in `install.py`, in this order: tracked-branch check via `_tracked_branches.is_tracked()` (FR-013), `graphify` on PATH check (FR-011), hook-collision check for both target hook paths (FR-014) — each refusal makes no other changes (depends on T003, T017)
- [X] T019 [US4] Implement installer actions in `install.py`: write both hooks with the resolved shebang, add a `graphify-out/` line to `.gitignore` if absent (FR-017), prompt for confirmation before invoking `graphify install` on the operator's behalf (FR-012) (depends on T018)
- [X] T020 [P] [US4] Unit tests for the installer in `tests/atoms/graphify_first_authoring/test_install.py`: each precondition's refusal path leaves no partial changes, a successful run writes both hooks plus the `.gitignore` line, the confirmation prompt gates the `graphify install` subprocess call (depends on T019)
...
- [ ] T021 [P] Write `.specify/atoms/graphify-first-authoring/README.md` (operator adoption docs, mirroring [quickstart.md](quickstart.md))
...
- [ ] T023 Add the new `atoms[]` entry to the repo-root `.haex-hive.json` for haex-hive's own self-adoption (data-model.md §ConsumerManifestEntry) — depends on T022. **This is Commit 2 of the two-commit self-adoption convention**: its `revision` field pins the exact SHA of T022's Commit 1, which is already in git history at this point
- [ ] T024 Run `haex constitution assemble` and review the merged `.haex-hive/constitution.md` (this exercises the multi-source LLM-merge path, since haex-hive now has two constitution-contributing atoms) — depends on T023
- [ ] T025 Run `haex constitution show` and verify the "Assembled from" preface names both source atoms — depends on T024
- [ ] T026 [P] Run the full [quickstart.md](quickstart.md) walkthrough end-to-end on a scratch clone
- [ ] T027 [P] Run `ruff` and `mypy` (per the repo's existing `pyproject.toml` tool config) over all new atom scripts and tests
- [ ] T028 [P] Append a `graphify-out/` line to the repo-root `.gitignore` so haex-hive's own self-adoption does not surface the artifact directory in subsequent working-tree operations — this is the direct enforcement of FR-009 for this repo (rather than routing through the installer's `.gitignore` write, which targets *other* consumers)
