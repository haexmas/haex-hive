...
- [X] T008 [US1] Manually verify via [quickstart.md](quickstart.md) steps 3–5 (adopt in `.haex-hive.json`, `haex constitution assemble`, `haex constitution show`) that the contributed text is correctly picked up — this is a behavioral check, not a unit test — **verified transitively by T024/T025 on this repo's own self-adoption**: `haex constitution show` prints an "Assembled from" preface naming both source atoms and the assembled body contains the "Principle: graphify-first authoring" heading
